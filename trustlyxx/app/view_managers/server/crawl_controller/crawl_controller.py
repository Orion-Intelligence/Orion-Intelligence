...
from app.services.elastic_manager.elastic_enums import ELASTIC_REQUEST_COMMANDS, ELASTIC_INDEX
...
from app.backend.view_managers.server.crawl_controller.crawl_enums import CRAWL_COMMANDS, CRAWL_ERROR_CALLBACK
from app.backend.view_managers.server.crawl_controller.crawl_session_controller import crawl_session_controller
...
class crawl_controller(request_handler):
  ...
  def __handle_request(self, p_data):
    ...
    if m_status is False:
      ...
      m_context = {}
      mongo_controller.get_instance().invoke_trigger(MONGODB_CRUD.S_UPDATE, [MONGO_COMMANDS.M_UPDATE_STATUS, ["m_crawler"], [None]])
      ...
      if m_leak_index and len(m_leak_index["cards_data"]):
        ...
        mongo_controller.get_instance().invoke_trigger(MONGODB_CRUD.S_UPDATE, [MONGO_COMMANDS.M_UPDATE_URL_STATUS, [m_leak_index["base_url"], True, len(m_leak_index["cards_data"]) > 0, m_leak_index["content_type"], m_leak_index["m_network"]], [None]])
      ...
      if m_generic_index:
        ...
        mongo_controller.get_instance().invoke_trigger(MONGODB_CRUD.S_UPDATE, [MONGO_COMMANDS.M_UPDATE_URL_STATUS, [m_generic_index["m_base_url"], True, None, m_generic_index["m_content_type"], m_generic_index["m_network"]], [None]])
  ...
  def __handle_publish_feeder(self, p_data):
    m_status, m_crawl_model = self.__m_session.invoke_trigger(CRAWL_COMMANDS.M_INIT, p_data)
    m_crawl_model.m_data = json.loads(m_crawl_model.m_data)
    file_path = os.path.join(settings.BASE_DIR, 'static', 'backend', '.well-known', 'feeder', "crawl_data_unique.txt")
  ...
  def invoke_trigger(self, p_command, p_data):
    ...
    if p_command == CRAWL_COMMANDS.M_FETCH_FEEDER:
      file_path = os.path.join(settings.BASE_DIR, 'static', 'backend', '.well-known', 'feeder', "crawl_data.txt")
    ...
    if p_command == CRAWL_COMMANDS.M_FETCH_PARSER:
      parser_folder = os.path.join(settings.BASE_DIR, 'static', 'backend', '.well-known', 'parser')
      ...
      with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for root, dirs, files in os.walk(parser_folder):
          for dir_name in dirs:
            ...
            arcname = os.path.relpath(dir_path, start=parser_folder)

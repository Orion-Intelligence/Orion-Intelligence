...
describe('Orion Intelligence - Search Navigation and Report Access', () => {
  ...
  cy.visit('https://try.orionintelligence.org/dashboard/exploit/all?page=1');
  waitForSearchReady();
  typeExploitSearch('exploit');
  clickOpenReport();

  cy.go('back');
  cy.location('pathname').should('eq', '/dashboard/exploit/all');
  waitForSearchReady();

  cy.visit('https://try.orionintelligence.org/dashboard/exploit/cve?page=1');
  waitForSearchReady();
  typeExploitSearch('cve');
  cy.get('[data-testid="open-report"]')
    .filter(':visible')
    .filter(':has(img[src*="redirect.svg"])')
    .first()
    .scrollIntoView()
    .should('be.visible')
    .click();
  ...
  cy.go('back');
  cy.location('pathname').should('eq', '/dashboard/exploit/cve');
  waitForSearchReady();

  cy.visit('https://try.orionintelligence.org/dashboard/exploit/tools?page=1');
  waitForSearchReady();
  typeExploitSearch('tool');
  clickOpenReport();

  cy.go('back');
  cy.location('pathname').should('eq', '/dashboard/exploit/tools');
  waitForSearchReady();

  cy.visit('https://try.orionintelligence.org/dashboard/exploit/zeroday?page=1');
  waitForSearchReady();
  typeExploitSearch('exploit');
  clickOpenReport();

  cy.go('back');
  cy.location('pathname').should('eq', '/dashboard/exploit/zeroday');
  waitForSearchReady();

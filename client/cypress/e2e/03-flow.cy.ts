describe('Orion Intelligence – Shared Session', {testIsolation: false}, () => {
  ...
  describe('Orion Intelligence – Full Stable Flow', () => {
    function openSidebarGroup(title: string) {
      ...
      cy.contains('app-dashboard-sidebar-items > li > div', title).closest('li').find('> ul', {timeout: 20000}).should(($ul) => {
    ...
    function clickSidebarSubItem(groupTitle: string, itemTitle: string) {
      cy.contains('app-dashboard-sidebar-items > li > div', groupTitle).closest('li').find('> ul').should(($ul) => {
        expect(getComputedStyle($ul[0] as HTMLElement).pointerEvents).not.to.equal('none');
      }).contains('li > div > div', new RegExp(`^\\s*${itemTitle}\\s*$`)).scrollIntoView().click();
    }

    it('Full Flow', () => {
      cy.contains('app-dashboard-sidebar-items div', 'admin').scrollIntoView().click();
      ...
      [
        ...
        cy.contains('app-dashboard-sidebar-items div', section).scrollIntoView().click();
      });

      cy.get('img[src*="nav_menu"]').should('be.visible').click();

      cy.get('img[src*="menu-mini"]').should('be.visible').click();
  ...
  describe('Orion Intelligence - Heatmap Coverage', () => {
    ...
    it('covers world heatmap render, interactions and popup close paths', () => {
      ...
      cy.get('app-world-heatmap .map-container path.country.has-data, app-world-heatmap .map-container path.country').first().should('exist').then(($el) => {

describe('Dashboard Sections Test – Stealer Logs', () => {
  ...
  it('Stealer Logs → Credential & Logs flow', () => {
    cy.visit('/dashboard');
    ...
    cy.get('.credential-table table tbody tr')
      .should('have.length.greaterThan', 0);


    cy.get('button.expand-btn')
      .should('have.length.greaterThan', 0)
      .each(($btn) => {
    ...
    cy.contains('.sidebar__subitem-content', 'Logs')
      .should('be.visible')
      .click();
...
it('Live APIs → Email Breach, Social Scanner, Playstore Scanner, Software Scanner', () => {
  ...
  cy.get('.dash-search-button').click();
  ...
  cy.get('.consolidated-ioc-container')
    .should('exist')
    .and('be.visible');


  cy.contains('.consolidated-ioc-stat-label', 'Search Elapsed Time')
    .should('exist');

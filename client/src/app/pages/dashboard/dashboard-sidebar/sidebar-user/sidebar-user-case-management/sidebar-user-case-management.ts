...
export class SidebarUserCaseManagement implements OnInit {
  ...
  getStatusBadgeClass(status: string): string {
    ...
    if (status === 'open') return isLightTheme ? 'bg-blue-100 text-blue-800' : 'bg-blue-500/10 text-blue-300';
    if (status === 'in-progress') return isLightTheme ? 'bg-yellow-100 text-yellow-800' : 'bg-yellow-500/10 text-yellow-300';
    if (status === 'resolved') return isLightTheme ? 'bg-emerald-100 text-emerald-800' : 'bg-emerald-500/10 text-emerald-300';
    return isLightTheme ? 'bg-slate-100 text-slate-700' : 'bg-slate-500/10 text-slate-300';
  }

  getPriorityBadgeClass(priority: string): string {
    const isLightTheme = document.body.classList.contains('light-theme');
    if (priority === 'critical') return isLightTheme ? 'bg-red-100 text-red-800' : 'bg-red-500/10 text-red-300';
    if (priority === 'high') return isLightTheme ? 'bg-orange-100 text-orange-800' : 'bg-orange-500/10 text-orange-300';
    if (priority === 'medium') return isLightTheme ? 'bg-yellow-100 text-yellow-800' : 'bg-yellow-500/10 text-yellow-300';

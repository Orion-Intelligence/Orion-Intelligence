...
export class ScannerService {
  ...
  scanDomain(domain: string, scanType: string): Observable<any> {
    ...
    return this.api.post<any>('urlscan/domain', body).pipe(
      takeUntil(this.cancel$),
      expand((res: any) =>
        this.isPending(res)
          ? timer(5000).pipe(
              takeUntil(this.cancel$),
              switchMap(() => this.api.post<any>('urlscan/domain', body).pipe(takeUntil(this.cancel$)))
            )
          : EMPTY
      ),
      takeWhile((res: any) => this.isPending(res), true)
    );
  }

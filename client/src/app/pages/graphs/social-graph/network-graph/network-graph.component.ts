...
export class NetworkGraphComponent implements OnInit, OnDestroy {
    ...
    private isNodeMatch(node: any, originalNode: any, originalNodes: any[], lowerCaseTerm: string, normalizedTerm: string, matchedUserNodeIds: Set<string>): boolean {
    ...
    private createNetwork(containerEl: HTMLElement, data: {
        nodes: DataSet<any>;
        edges: DataSet<any>;
    }) {
        this.destroyNetwork();
    ...
    trackByOverlayNodeId(_index: number, item: {
        nodeId: string;
    }): string {
        return item.nodeId;
    }

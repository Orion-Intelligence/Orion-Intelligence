...
export class WantedListSectionComponent implements OnDestroy {
  ...
  profileKey = computed(() => SocialNormalizationUtil.normalizeUsername(this.username()));
  ...
  search(event?: Event): void {
    ...
    this.records.set([]);
    this.saveProfileState();
    this.subscription = this.state.fetchWantedList(query).subscribe({
      next: (records) => {
  ...
  getRecordTitle(record: any): string {
    return SocialNormalizationUtil.firstValue(record?.name, record?.caption, record?.entity, record?.id) || 'Unknown person';
  }

  getRecordMeta(record: any): string {
    return SocialNormalizationUtil.firstValue(record?.schema, record?.status, record?.authority, record?.program, record?.topics, record?.datasets) || '-';
  }

  getRecordSummary(record: any): string {
    return SocialNormalizationUtil.firstValue(record?.description, record?.summary, record?.notes, record?.source_url) || '';
  }
}

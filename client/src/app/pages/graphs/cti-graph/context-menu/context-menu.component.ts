...
@Component({
  ...
  <div id="customContextMenu" data-testid="cti-context-menu" class="ui-graph-flyout hidden fixed z-[1000] w-64 rounded-xl border border-slate-700/80 bg-slate-900/95 backdrop-blur-sm shadow-2xl focus:outline-none animate-[social-mapper-slide-down_0.12s_ease-out_forwards]">
    ...
    <div class="p-2">
      <button data-testid="cti-context-expand" *ngIf="canExpand" (click)="expand.emit()" class="group flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm text-slate-200 transition-colors hover:bg-indigo-600/30 hover:text-white">
      ...
      <button data-testid="cti-context-collapse" *ngIf="canCollapse" (click)="collapse.emit()" class="group flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm text-slate-200 transition-colors hover:bg-slate-700/70 hover:text-white">
      ...
      <button data-testid="cti-context-open-cti" *ngIf="showOpenCti" (click)="openCti.emit()" class="group flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm text-sky-200 transition-colors hover:bg-sky-500/20 hover:text-sky-100">
      ...
      <button data-testid="cti-context-copy-label" *ngIf="showCopyLabel" (click)="copyLabel.emit($event)" class="group flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm text-slate-200 transition-colors hover:bg-slate-700/70 hover:text-white">
      ...
      <button data-testid="cti-context-open-report" *ngIf="showOpenReport" (click)="openReport.emit()" class="group flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm text-red-300 transition-colors hover:bg-red-500/20 hover:text-red-200">

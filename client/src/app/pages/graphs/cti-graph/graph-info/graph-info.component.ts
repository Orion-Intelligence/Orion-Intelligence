...
export class GraphInfoComponent implements OnInit {
    ...
    @Input()
    expandEnabledTemp!: boolean;
    @Input()
    result: any[] = [];
    @Input()
    ruleSet!: RuleSet;
    @Output()
    ruleSetChange = new EventEmitter<RuleSet>();
    @Output()
    physicsToggled = new EventEmitter<boolean>();
    @Output()
    expandToggled = new EventEmitter<boolean>();
    @Output()
    onResetAll = new EventEmitter<void>();
    detailsOpen = true;
    ...
    toggleCollapse(section: 'details' | 'indicators' | 'ruleset') {
        if (section === 'details') {
            ...
            this.rulesetOpen = !this.rulesetOpen;
        }
    }
    toggleAnimation() {

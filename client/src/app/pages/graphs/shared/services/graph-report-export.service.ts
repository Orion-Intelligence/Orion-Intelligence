...
export class GraphReportExportService {
  exportByType(payload: GraphReportPayload, type: GraphReportExportType): void {
    if (type === 'json') { this.exportGraphJson(payload); return; }
    if (type === 'graph_pdf') { const bytes = this.buildGraphPdfBytes(payload); this.downloadBinary(bytes, 'application/pdf', `${this.buildSafeFilename(payload)}-graph-report.pdf`); return; }
    const bytes = this.buildDocPdfBytes(payload); this.downloadBinary(bytes, 'application/pdf', `${this.buildSafeFilename(payload)}-doc-report.pdf`);
  }
  ...
  private drawGraphCover(doc: jsPDF, payload: GraphReportPayload, meta: { generatedAt: string; kindLabel: string }): void {
  ...
  private drawGraphToc(
    doc: jsPDF,
    payload: GraphReportPayload,
    pages: { graphPage: number | null; analysisPage: number; platformPage: number | null; edgesPage: number }
  ): void {
    doc.setPage(1);
    const W = this.getPageW(doc);
    ...
    const sectionItems: Array<{ label: string; page: number }> = [];
    if (pages.graphPage) {
  ...
  private drawGraphAnalysisHeader(doc: jsPDF, payload: GraphReportPayload, meta: { generatedAt: string; kindLabel: string }): void {
    const W = this.getPageW(doc);
    doc.setFillColor(15, 23, 42);
  ...
  private drawGraphFooter(
    doc: jsPDF,
    payload: GraphReportPayload,
    meta: { generatedAt: string; kindLabel: string },
    pageNo: number,
    totalPages: number
  ): void {
    const W = this.getPageW(doc);
  ...
  private drawCover(doc: jsPDF, payload: GraphReportPayload, meta: { generatedAt: string; kindLabel: string }, subtitle: string): void {
    const W = this.getPageW(doc);
    doc.setFillColor(15, 23, 42);
  ...
  private makeHeaderFooterHooks(payload: GraphReportPayload, meta: { generatedAt: string; kindLabel: string }): { didDrawPage: (data: any) => void; drawHeader: (doc: jsPDF, section: string) => void; drawFooter: (doc: jsPDF, meta: any) => void } {
    ...
    const docPageNumberSafe = (d?: jsPDF): number => {
      try { return d?.getCurrentPageInfo().pageNumber ?? 1; } catch { return 1; }
    };

    return { didDrawPage, drawHeader, drawFooter };
  }

  private buildTypeComposition(nodes: GraphReportNode[]): { type: string; count: number }[] {
  ...
  private extractSocialPlatformCounts(payload: GraphReportPayload): Array<{ name: string; count: number }> {
    if (payload.graphKind !== 'social') {
      return [];
    }
  ...
  private makeMeta(payload: GraphReportPayload): { generatedAt: string; kindLabel: string } {
  ...
  private fitRectToPage(doc: jsPDF, left: number, top: number, right: number, bottom: number): { x: number; y: number; w: number; h: number } {
    const W = this.getPageW(doc);
    const H = this.getPageH(doc);
    return { x: left, y: top, w: W - left - right, h: H - top - bottom };
  }

  private fitRect(_: jsPDF, maxW: number, maxH: number, x: number, y: number): { x: number; y: number; w: number; h: number } {

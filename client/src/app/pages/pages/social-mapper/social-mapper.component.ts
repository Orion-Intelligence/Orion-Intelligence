import { Component, ChangeDetectionStrategy, signal, computed, DestroyRef, OnDestroy, PLATFORM_ID, effect, OnInit, viewChild, ElementRef, Inject, inject } from '@angular/core';
...
import { NetworkData, Job, PlatformResult, CustomEntity, NetworkNode, TabState, ProfileDetails, SocialImage, ScanEvent, SocialPost, GraphPlatformBatch } from '../../shared/model/social/social-scan.models';
...
import { ManageProfilesModalComponent, ManageProfilesModalData } from './components/manage-profiles-modal/manage-profiles-modal.component';
...
import { ContextMenuComponent, ContextMenuData, ContextMenuAction } from './components/context-menu/context-menu.component';
import { NotificationBarComponent, NotificationData, NotificationType } from './components/notification-bar/notification-bar.component';
...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, TitleCasePipe, NetworkGraphComponent, MetadataPopupComponent,
    ProfileSummaryPopupComponent, ToolbarComponent, HomeMenuComponent,
    ListViewComponent, TabBarComponent, SocialIconComponent,
    FollowerScanPopupComponent, ManageProfilesModalComponent, ConfirmationModalComponent,
    InfoModalComponent, ContextMenuComponent, NotificationBarComponent,
...
export class SocialMapperComponent implements OnInit, OnDestroy {
  ...
  expandedGroupDataByUser = computed(() => this.activeTabState()?.expandedGroupDataByUser() ?? {});
  graphPlatformBatches = computed(() => this.activeTabState()?.graphPlatformBatches() ?? new Map());
  ...
  clearGraphSearch() { this.graphSearchTerm.set(''); }
  ...
  toggleMobileEntityPanel(event?: MouseEvent): void { event?.stopPropagation(); this.isMobileEntityPanelOpen.update(v => !v); }
  closeMobileEntityPanel(): void { this.isMobileEntityPanelOpen.set(false); }
    ...
    return {
      next: (event: ScanEvent) => {
        if (event.type === 'progress') {
          ...
          if (isImageScan) this.state.openManageProfilesModal(job.username);
        }
      },
      error: (err: any) => {
  ...
  private resumeIncompleteScans() {
    this.jobs().filter(job => job.status === 'in_progress').forEach(job => {
      if (!this.cancelScanSubjects.has(job.id)) this.runScan(job);
    });
  }
  ...
  private fetchData(platformResult: PlatformResult, stateKey: keyof FetchingStateService, request$: Observable<any>, cancelMap: Map<string, Subject<void>>) {
    ...
    if (this.fetchingState.isUserBusy(platformResult.keyUsername)) { this.state.showNotification('busy'); return; }
    if (cancelMap.has(key)) return;
        ...
        next: (response: any) => {
          ...
          this.updateState(state => {
            state.scanResults.update(currentMap => {
              ...
              if (userResults) newMap.set(platformResult.keyUsername, userResults);
              return newMap;
            });
          });
          this.updateUIPopups(platformResult, newData);
          if (stateKey === 'followers' || stateKey === 'following') {
            this.graphOrchestrator.updateUserConnections(this.activeTabState()!);
          }
        },
        error: (err) => {},
        complete: () => {
  ...
  private updateUIPopups(p: PlatformResult, data: Partial<PlatformResult>) {
    if (this.state.selectedPlatformData()?.url === p.url) this.state.selectedPlatformData.update(current => current ? { ...current, ...data } : null);
    ...
    if (this.state.followerScanPopupData()?.platform.url === p.url) this.state.followerScanPopupData.update(current => current ? { platform: { ...current.platform, ...data } } : null);
  }
  ...
  onPlatformNodeClicked(nodeId: string) {
    if (this.isCustomEntityNode(nodeId) || !nodeId.startsWith('platform-')) return;
  ...
  async handleGroupNodeClicked({ nodeId, position }: { nodeId: string, position: Position }) {
    ...
    if (!wasPhysicsEnabled) this.updateState(state => state.isPhysicsEnabled.set(true));
    
    await this.graphOrchestrator.handleGroupNodeClicked(this.activeTabState()!, { nodeId, position });

    if (!wasPhysicsEnabled) setTimeout(() => this.updateState(state => state.isPhysicsEnabled.set(false)), 2500);
  }
}
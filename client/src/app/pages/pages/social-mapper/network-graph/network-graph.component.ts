...
import { NetworkData, PlatformResult, NetworkNode } from '../../../shared/model/social/social-scan.models';
...
export class NetworkGraphComponent implements OnInit, OnDestroy {
    ...
    for (const state of statesToProcess) {
      if (state) {
        for (const key in state) {
          if (state[key]) {
            loadingIds.add(key); // key is now the node ID
  ...
  constructor() {
    ...
    effect(() => {
        ...
        const network = this.networkInstance();
        if (!network || this.visData.nodes.length === 0) return;
        ...
        if (!lowerCaseTerm) {
            ...
            this.visData.nodes.forEach((node: any) => {
                ...
                let isMatch = false;
                if (node.label && node.label.toLowerCase().includes(lowerCaseTerm)) {
                    isMatch = true;
                } else if (originalNode.groupedPlatforms) {
                    isMatch = originalNode.groupedPlatforms.some((p: PlatformResult) =>
                        (p.platform && p.platform.toLowerCase().includes(lowerCaseTerm)) ||
                        (p.username && p.username.toLowerCase().includes(lowerCaseTerm))
                    );
                }
                ...
                if (isMatch) {
                    updates.push({
                        id: node.id,
                        color: { ...originalNode.color, border: '#facc15' }, // Yellow border
  ...
  private updateNetworkData(networkData: NetworkData) {
    ...
    if (nodesToRemove.length > 0) this.visData.nodes.remove(nodesToRemove);
    if (nodesToAdd.length > 0) this.visData.nodes.add(nodesToAdd);

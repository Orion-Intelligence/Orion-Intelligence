...
export class FetchingStateService {
  profile = signal<{ [key: string]: boolean }>({});
  posts = signal<{ [key: string]: boolean }>({});
  platformImages = signal<{ [key: string]: boolean }>({});
  followers = signal<{ [key: string]: boolean }>({});
  following = signal<{ [key: string]: boolean }>({});

  userImages = computed(() => {
    const platformState = this.platformImages();
    const userState: { [username: string]: boolean } = {};
  ...
  isUserBusy(username: string): boolean {
    if (this.userImages()[username]) return true;
    ...
    return states.some(state => 
      Object.keys(state).some(key => 
        key.startsWith(userNodeIdPrefix) && state[key]
      )
    );
  }

...
export class CasePdfExportService {
  exportCaseReport(report: SharedCaseReport, options: CasePdfExportOptions = {}): Observable<void> {
    ...
    }).pipe(map(({ jspdfModule, autoTableModule }) => {
      const doc = new jspdfModule.default({ orientation: 'portrait', unit: 'pt', format: 'a4', compress: true });
      this.buildPdf(doc, autoTableModule.default, report, options);
  ...
  private drawPdfCover(doc: jsPDF, report: SharedCaseReport, options: CasePdfExportOptions): void {
    ...
    const appName = options.appName || 'Orion Intelligence';
    ...
    doc.setFontSize(20);
    doc.text(this.fitPdfLine(doc, report.title || reportLabel, pageWidth - 80), 40, 48);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.setTextColor(254, 202, 202);
    doc.text(`${reportLabel} · ${report.caseId} · ${this.formatLabel(report.caseType)}`, 40, 68);

...
export class CasePdfExportService {
  ...
  exportCaseReport(report: SharedCaseReport, options: CasePdfExportOptions = {}): Observable<void> {
    return forkJoin({
      ...
      doc.save(`${this.safeFilename(report.caseId || 'case')}-${options.filenameSuffix || 'shared-report'}.pdf`);
    }));
  }
  ...
  private drawPdfCover(doc: jsPDF, report: SharedCaseReport, options: CasePdfExportOptions): void {
    ...
    doc.setFontSize(20);
    doc.text(this.fitPdfLine(doc, report.title || reportLabel, pageWidth - 280), 40, 48);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.setTextColor(254, 202, 202);
    doc.text(this.fitPdfLine(doc, `${reportLabel} · ${report.caseId} · ${this.formatLabel(report.caseType)}`, pageWidth - 280), 40, 68);
    doc.text(this.fitPdfLine(doc, `Generated by ${appName}${report.expiresAt ? ` · Expires ${this.formatDate(report.expiresAt)}` : ''}`, pageWidth - 80), 40, 86);
    this.drawTenantBrand(doc, appName, options.logoDataUrl || null, pageWidth - 40, 20, 130, 24);
  }
  ...
  private addPdfSection(doc: jsPDF, autoTable: typeof import('jspdf-autotable').default, y: number, title: string, rows: RowInput[], contentWidth: number): number {
    ...
    autoTable(doc, {
      ...
      styles: {
        ...
        textColor: [30, 41, 59],
        lineWidth: 0.2,
        lineColor: [229, 199, 199],
      },
      bodyStyles: {
        fillColor: [255, 251, 251],
        lineWidth: 0.2,
        lineColor: [229, 199, 199],
      },
      alternateRowStyles: {
        fillColor: [254, 247, 247],
        lineWidth: 0.2,
        lineColor: [229, 199, 199],
      },
  ...
  private drawTenantBrand(doc: jsPDF, appName: string, logoDataUrl: string | null, rightX: number, topY: number, maxWidth: number, maxHeight: number): void {
    let textY = topY + 10;
    if (logoDataUrl) {
      try {
  ...
  private formatLabel(value?: string | null, otherValue?: string | null): string {
    ...
    return String(this.exportBranding.replaceSystemBrand(label));
  }

...
export class CredentialComponent implements OnInit, AfterViewInit {
    ...
    fetchSearchResults(reset = true): void {
        ...
        this.dashboardService
            ...
            .pipe(switchMap(response => timer(300).pipe(map(() => response))), finalize(() => { this.setLoading(-1), this.isSearchLoading = false; }))
            .subscribe(response => {
    ...
    fetchRanked() {
        ...
        this.dashboardService
            ...
            .pipe(switchMap(response => timer(500).pipe(map(() => response))), finalize(() => { this.setLoading(-1), this.isRankedLoading = false, this.dashboardService.consolidatedParamModel.ioc = ''; }))
            .subscribe(response => {
            const endTime = performance.now();

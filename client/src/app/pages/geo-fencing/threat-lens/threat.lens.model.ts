...
export interface ThreatLensIotMarkerCluster {
  id: string;
  latitude: number;
  longitude: number;
  count: number;
...
export interface ThreatLensCoordinate {
  lat: number;
  lng: number;
}

export interface ThreatLensVisibleArea {
  center: ThreatLensCoordinate;
  bounds: {
    minLat: number;
    maxLat: number;
    westLng: number;
  ...
  radiusKm: number;
  sampledPoints: ThreatLensCoordinate[];
}

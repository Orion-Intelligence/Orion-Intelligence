...
export class ThreatResultsComponent implements OnInit, OnChanges {
    @Input()
    results_defacement!: DefacementCallbackModel | undefined;
    @Input()
    results_stealerlog!: StealerLogCallbackModel | undefined;
    @Input()
    isExpandable = false;
    ...
    webServerValue(item: any): string {
        if (item?.m_web_server?.length) {
            return item.m_web_server.join(', ');
        }
        return '-';
    }
    attackerValue(item: any): string {
        if (item?.m_attacker?.length) {

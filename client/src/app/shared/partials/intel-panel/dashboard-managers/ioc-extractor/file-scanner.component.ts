...
import { IocExtractionResult, IocItem, GroupedIoc, ApkScanResult, SummaryItem } from '../../../../model/ioc-extractor/ioc.extractor.model';
...
export class FileScannerComponent {
  ...
  private processApkToIocFormat(apk: ApkScanResult): GroupedIoc[] {
    const categorized = new Map<string, { key: string; items: any[] }>();

    if (apk.network?.sample_urls?.length > 0) {
      const key = 'm_url';
      if (!categorized.has(key)) categorized.set(key, { key, items: [] });
  ...
  exportReport(): void {
    if (!this.extractionResult && !this.apkResult) return;

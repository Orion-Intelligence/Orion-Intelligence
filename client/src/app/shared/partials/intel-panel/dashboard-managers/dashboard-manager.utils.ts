...
export function applyQueryAndPageFromParams(
  params: QueryPageSource,
  target: QueryPageParams
): string {
...
export function setPageAndFetch(
  target: Pick<QueryPageParams, 'page'>,
  page: number,
  fetch: () => void
): void {
  target.page = page;
...
export function resetPageAndFetch(
  target: Pick<QueryPageParams, 'page'>,
  fetch: () => void
): void {
  target.page = 1;
...
export function resolveSortOrder(sort: SortType): SortOrder | null {
  if (sort === SortType.NEWEST_FIRST) return 'desc';
  if (sort === SortType.OLDEST_FIRST) return 'asc';
  return null;
}

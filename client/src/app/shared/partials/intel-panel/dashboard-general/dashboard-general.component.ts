...
@Component({
  ...
  imports: [NgIf, PaginationComponent, DashboardResultsGridComponent, ResultComponent, DashboardResultChatComponent],
...
export class DashboardGeneralComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.dashboardService.consolidatedParamModel = {...this.dashboardService.consolidatedParamModel} as ConsolidatedParamModel;
    ...
    combineLatest([this.route.queryParams, this.route.url])
      ...
      .subscribe(([params, urlSegments]) => {
        ...
        this.dashboardService.consolidatedParamModel.mSearchParamSafeSearch = params['mSearchParamSafeSearch'] === 'true';
        this.dashboardService.consolidatedParamModel.mDateRange = params['mDateRange'] || '';
        this.dashboardService.consolidatedParamModel.mNetwork = params['mNetwork'] || 'all';
  ...
  resetFilters(_: void) {
    this.dashboardService.consolidatedParamModel.mSearchParamType = "all";
    this.dashboardService.consolidatedParamModel.mSearchParamPage = 1;
    this.dashboardService.consolidatedParamModel.mSearchParamSafeSearch = false;
    this.dashboardService.consolidatedParamModel.mNetwork = "all";
  ...
  reloadFilters(filters: Record<string, string | null>) {
    ...
    if (filters['mNetwork'] != null) {
      this.dashboardService.consolidatedParamModel.mNetwork = filters['mNetwork'];
    }
    if (filters['mDateRange']) {
      this.dashboardService.consolidatedParamModel.mDateRange = filters['mDateRange']
    }
    if (filters['mContentType'] != null) {
      this.dashboardService.consolidatedParamModel.mContentType = filters['mContentType'];
    }
    if (filters['mEntity'] != null) {
      this.dashboardService.consolidatedParamModel.mEntity = filters['mEntity'];
    }
    this.dashboardService.consolidatedParamModel.mSearchParamSafeSearch = filters['mSearchParamSafeSearch'] === 'yes';
    this.fetchSearchResults();
  }

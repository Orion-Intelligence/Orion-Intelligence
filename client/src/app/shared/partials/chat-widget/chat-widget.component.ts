...
export class ChatWidgetComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    bottomSentinel!: ElementRef<HTMLElement>;
    chatMessages: {
        id: string;
    ...
    private setupObserversIfPossible(): void {
        ...
        if (!this.io) {
            this.io = new IntersectionObserver(entries => { this.userNearBottom = entries.some(e => e.isIntersecting); }, { root: rootEl, threshold: 1 });

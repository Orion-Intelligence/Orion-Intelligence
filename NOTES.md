...
## System backup blocks all tenants and redirects to the maintenance page (2026-09-22)

An admin-initiated instant/system backup now holds the whole site in maintenance for the length of the backup, the same way a restore always did, and the admin who starts a backup or a full restore is sent straight to `/static/maintenance.html` instead of watching an in-page progress bar. `start_backup` raises the `.maintenance` flag (touch + `maintenance_state.invalidate()`) right after the job store `begin`, before spawning `_run_backup(..., maintenance=True)`, and `_run_backup`'s `finally` clears it again; the scheduled/cron path (`run_backup_now` → `_run_backup` with `maintenance=False`) deliberately does NOT block, so nightly backups do not take every tenant offline. Because a backup writes no `restore_marker`, a crash mid-backup would strand the flag, so startup recovery calls `BackupManager.clear_stale_backup_maintenance()` (under the existing `backup:startup_recovery` Redis lock, after `resolve_interrupted_restore`/`resolve_interrupted_tenant_restore`); it removes the flag only when there is no restore marker, and the Mongo-backed job store self-heals the phantom "running" job through its stale-expiry. Client side, `confirmInstantBackup` (admin-only button) and `confirmRestoreBackup` (only when `!isTenantScope`) `window.location.assign('/static/maintenance.html')` once the start POST returns with the job registered as running; tenant-scope restore keeps the tenant-fence + interceptor flow. `maintenance.html`'s poll now leaves the page only when the job is not running AND the site is back online (`if (!running && await isBackOnline())`): the running guard is what holds the page on `:4200`, where there is no nginx and FastAPI exempts `/api/public` so `isBackOnline()` is always true, and it also prevents a bounce in the split second between the redirect and the flag/job going live. nginx exempts `/api/admin/backups/status` (progress keeps rendering during maintenance) while gating `/api/public` (page holds in production).

## Maintainers get Orion Mail by default and can grant it at user creation (2026-09-22)

Orion Mail access for a user is decided client-side by `LicenseService.canUseOrionMail()` = admin OR any maintainer (`isMaintainer()`, primary or secondary) OR a user whose `permissions` contain `orion_mail`, so every maintainer already has the mail entry by default without holding the permission. There is no backend gate on the `ORION_MAIL` permission: the SSO `authorize` only needs an active session, `_identity_for_user` passes no role/permission to Orion Mail, and the Orion-mail side provisions the mailbox lazily and ungated on first SSO (`mailbox_manager.create_mailbox`); the permission's only backend effect is `AccountManager._delete_orion_mail_account`, which removes the mailbox on user delete when the user held `orion_mail`. A maintainer can already grant `orion_mail` to its own tenant's users because `create_user`/`update_user` set `permissions` freely for a maintainer (only `MONITORING` is admin-gated via `_assert_monitoring_allowed`). The one gap was UI parity: the edit-user form (`manage-profile.component` `permissionOptions`) always offered the Orion Mail option, but the add-user form (`add-tenant.component`, which posts `tenant/create/user` for both admin and maintainer) did not, so mail could only be granted after creating the user. `add-tenant.component.permissionOptions` now also pushes `{ key: 'orion_mail' }`, matching the edit form, so a maintainer (and admin) can assign mail access at creation time.

## Homepage map flash vs. the embedded homepage (2026-09-22)

`HomepageComponent` (selector `app-index`, route `/dashboard/home`) redirects admin/member/analyst/demo to `/dashboard/profile/homepage` and gates its own template on a `redirecting` flag so the map (`app-home-search`) never flashes before the redirect. The catch: `SidebarUserHomepageComponent` (the real `/dashboard/profile/homepage`) *renders `<app-index>` itself* (line ~167), so the same component runs a second time as the destination — where it must NOT redirect or gate, or the normal homepage's map disappears and the embedded copy fires a second `navigate()` into the in-flight one (`_NoMatch` uncaught rejection). Fix: the redirect only runs when `this.router.url.startsWith('/dashboard/home')` (the route, not the embed), and the navigate uses a leading slash (`['/dashboard/profile/homepage']`) like every other navigate in the app.
...
## Telegram promoted to its own "Telegram Check In Depth" category (2026-09-23)

Telegram used to be one sub-value of the Social category (`SocialSubCategory.TELEGRAM`, routed at `/dashboard/social/telegram` → `chat_model`). It is now its own top-level sidebar category titled **Telegram Check In Depth** (`Category.TELEGRAM = 'Telegram'` in `client/src/app/shared/constants/pages.ts`), with three sub-values in the new `TelegramSubCategory` enum — General / Stealerlog / Breaches — mirroring how Social is a `Category` with a `*SubCategory` enum. It was removed from `SocialSubCategory` and from the `social` route's `telegram` child (legacy `/dashboard/social/telegram` still resolves via the social `:category` catch-all → `chat_model`). Access reuses the `social` license: the sidebar `@if` gates on `canUseModule('social')` and `licenses.service.canAccess` maps `'Telegram' → 'social'` (same pattern as `'Stealerlogs' → 'stealer_logs'`), so no new license module was introduced.

The three sub-values map to three existing indices, each scoped to Telegram, reusing existing search + render paths (no new crawlers, no new renderers):

- **General → `chat_model`.** Route `/dashboard/telegram/general` uses the shared `DashboardResultContainer` with `data.type: 'social'`, so it POSTs `search/social`. `search_social` (backend `routes/api_routes.py`) now treats `category in ("telegram", "general")` the same → `base_index = [S_CHATS_INDEX]`. Rendered by the existing `chat_model` card; report route `general/:m_hash` → `loadReportChatComponent`.
- **Breaches → `leak_model` filtered to Telegram.** Route `/dashboard/telegram/breaches` also uses `type: 'social'` → `search/social`; the new `category == "breaches"` branch sets `param.network = "telegram"` and `base_index = [S_LEAK_INDEX]`. The telegram filter is the *existing* `m_network` term filter in `search_query_generator` (`{"term": {"m_network": m_network.lower()}}`), so it only works when telegram-sourced leaks are ingested with `m_network == "telegram"` (lowercase). `index_query_leak` already passes `m_network` straight through from the leak source card, so no crawler code change was needed — but a leak source that does not tag `m_network` telegram will not surface here. Rendered by the existing `leak_model` general-grid card; report route `breaches/:m_hash` → `loadReportComponent` (the leak report).
- **Stealerlog → `stealer_model`.** Route `/dashboard/telegram/stealerlog` loads the existing Stealer Logs UI (`loadCredentialComponent`, `data.type: 'credential'`) because `stealer_model` is the encrypted / 150-shard / IOC-only index with its own controller (`search_stealer_iocs`) and **no renderer in the consolidated `DashboardResultContainer`** — it cannot be surfaced through `search/social`. This sub-value therefore shows the standard stealer-logs search, NOT yet filtered to a telegram channel: the stealer index is keyed by IOC fields (domain/email/…) with only a raw `channel` keyword (the actual channel handle), so there is no boolean "telegram source" to filter on. True telegram-channel scoping of stealer logs is a follow-up that needs the stealer index/query to carry a telegram source/channel facet.

Collapsed-sidebar sub-icons: `General` already resolved to `sub_general.svg`; added `iconAliases` `stealerlog → stolen`, `breaches → leaks` in `dashboard-sidebar-collapsed.component.ts`. Tooltips for `Stealerlog`/`Breaches` added to `sidebarItemTooltips`.

## Per-tenant Orion Mail white-label (single deployment)

Orion Mail is a single deployment that each tenant SSOs into; the tenant's brand (name + logos) is what makes it "white-labeled", and it is derived from the tenant, not from env — always, with no per-tenant opt-in or override. No new tenant-doc schema — this reuses the existing per-tenant branding already exposed by `GET /api/public` (`app_name`, `logo_wide_light`, `logo_wide_dark`, resolved by Host in `tenant_resolution_middleware`). The mail URL itself stays a single global value (`ORION_MAIL_PUBLIC_URL`); it is not per-tenant.

- **Profile menu** (`profile.component`) no longer hardcodes "Orion Mail"; `mailLabel` = `<app_name> Mail` (falls back to "Mail").
- **Per-tenant mail sending** — the SSO identity now carries the tenant **slug** so Orion Mail can send each tenant from its own subdomain (`<slug>.mail.orionintelligence.org`). `sso_manager._identity_for_user` (now async) looks up the plaintext `db_tenant_model.slug` by `user.tenant_id` and adds `tenant_slug` to the identity returned by `authorize`/`exchange`/`verify`. The mail domain itself, DKIM (one org key via rspamd `use_esld`), Postfix multi-domain, and DNS all live on the Orion-mail side — see that repo's NOTES.
- The mail app reads the tenant brand from the Intelligence origin it was opened with (`orion_origin`); see the Orion-mail repo NOTES. The mail side inlines the two wide logos as `data:` URIs because the mail CSP is `img-src 'self' data:` (a cross-origin `<img>` to the Intelligence host would be blocked).

### Per-tenant webmail host (`<slug>.mail.orionintelligence.org`) (2026-09-23)

The branding (name/logo) + sending domain were already per-tenant; the last piece is the **browser URL** of the webmail itself being the tenant subdomain instead of the shared `mail.orionintelligence.org`. It is one shared mail deployment behind one wildcard — no per-tenant build, no per-tenant record beyond a single `*.mail` wildcard.

- **`profile.component.openOrionMail`** now rewrites the mail host from the tenant it is opened on: it takes the configured `orion_mail_url` host (`mail.<parent>`), and if the current Intelligence host is a direct subdomain of `<parent>` (e.g. `intelysync.orionintelligence.org` under parent `orionintelligence.org`) it prepends that first label → opens `https://intelysync.mail.orionintelligence.org/api/auth/login`. The `origin` query it sends is that per-tenant origin (so the mail callback returns to the tenant host). Localhost / bare-domain / non-subdomain hosts don't match → unchanged shared host. The slug equals the tenant subdomain label, which is the same slug `sso_manager._tenant_slug` puts on the identity, so the webmail host and the sending domain match (`intelysync.mail…`).
- **SSO redirect_uri** — `sso_manager._validate_redirect_uri` now accepts either the exact `S_ALLOWED_REDIRECT_URIS` set (local) **or** `SSO_CONSTANTS.S_ALLOWED_REDIRECT_URI_PATTERN` (env `ORION_MAIL_REDIRECT_URI_PATTERN`, default `^https://[a-z0-9-]+\.mail\.orionintelligence\.org/api/auth/callback$`). This is the code that lets the tenant callback host through; the mail side + Traefik do the rest.

No prod env change is required on the Intelligence side for this (the default pattern already covers `*.mail.orionintelligence.org`). All the remaining work is infra on the mail host — see the Orion-mail repo NOTES.

### Edge nginx for the per-tenant webmail host (2026-09-23)

DNS-only wildcard `A *.mail.orionintelligence.org → 144.76.157.34` is published in Cloudflare (tenant webmail bypasses the CF proxy and hits the origin edge nginx directly). The edge is `nginx/nginx-prod.conf` (the Intelligence prod nginx, also the box's :443 TLS terminator — base `mail.orionintelligence.org` stays CF-proxied on its existing block with the `try.orionintelligence.org` cert). Two problems it had for tenant hosts: nginx's leading wildcard `*.orionintelligence.org` (the app block) also matches `x.mail.orionintelligence.org`, so tenant mail hosts were landing on the Intelligence app with the wrong cert (mismatch) + 404.

Fix (in-repo): a new `server { listen 443 ssl; server_name *.mail.orionintelligence.org; }` block was added that proxies to `$mail_backend` (`orion-mail-nginx:8080`) with `Host $host` — identical to the base mail block except server_name + cert. nginx prefers the longest/most-specific leading wildcard, so `*.mail.orionintelligence.org` wins over `*.orionintelligence.org`. The mail app nginx (Orion-mail `nginx/nginx-prod.conf`) `server_name` now also lists `*.mail.orionintelligence.org` so it accepts the forwarded tenant Host (was falling to its `_` default → 444).

**One host-side step (cert is not in the repo) — DO THIS BEFORE deploying the nginx change**, or the edge nginx (which fronts the whole platform) fails to reload on a missing cert file:

```
# on the Intelligence prod box (144.76.157.34)
certbot certonly --dns-cloudflare \
  --dns-cloudflare-credentials /etc/letsencrypt/cloudflare.ini \
  --cert-name mail-wildcard \
  -d '*.mail.orionintelligence.org'
# lands at /etc/letsencrypt/live/mail-wildcard/ (matches the new nginx block)
```

A separate `--cert-name mail-wildcard` is deliberate: `/etc/letsencrypt/live/mail.orionintelligence.org/` already exists as the non-wildcard `mail.orionintelligence.org` cert (the MTA/SMTP cert) — reusing that name would collide with / overwrite it and would serve the wrong (non-wildcard) cert to tenant hosts. `cloudflare.ini` must hold the REAL Zone.DNS:Edit token (a leftover placeholder gives `6003 Invalid request headers`).

`cloudflare.ini` = `dns_cloudflare_api_token = <Zone.DNS:Edit token for orionintelligence.org>` (chmod 600). CAA already permits LE wildcards (`issuewild "letsencrypt.org"`). Add an nginx reload to the renewal deploy-hook. Only after the cert exists: `./run.sh build -p` on both boxes.

...
The webmail browser URL is the tenant's own **single-label** host `intelysyncmail.orionintelligence.org` (slug glued to `mail`), NOT the earlier three-level `<slug>.mail.…`. The switch to single-label is what makes this free: it is an ordinary two-level subdomain of `orionintelligence.org`, so the existing **Cloudflare proxied** `*.orionintelligence.org` wildcard DNS + CF Universal SSL already cover it — no `*.mail` DNS record, no `*.mail` wildcard cert, no certbot. One shared mail deployment, no per-tenant build.

- **`profile.component.openOrionMail`** rewrites the mail host from the tenant it is opened on: takes the configured `orion_mail_url` host (`mail.<parent>`), and if the current Intelligence host is a direct subdomain of `<parent>` it glues the first label straight on → `${slug}${mailHost}` = `intelysyncmail.orionintelligence.org` (was `${slug}.${mailHost}`). The `origin` query is that per-tenant origin so the callback returns to the tenant host. Localhost / bare-domain / non-subdomain hosts don't match → shared host unchanged.
- **SSO redirect_uri** — `sso_manager._validate_redirect_uri` accepts the exact `S_ALLOWED_REDIRECT_URIS` set (local) **or** `S_ALLOWED_REDIRECT_URI_PATTERN` (default now `^https://[a-z0-9-]+mail\.orionintelligence\.org/api/auth/callback$` — note the slug is glued onto `mail`, no `\.`).

### Edge nginx routing (the one non-obvious piece) (2026-09-23)

`nginx/nginx-prod.conf` is the box's :443 TLS terminator. The catch: `<slug>mail.orionintelligence.org` also matches the app block's `*.orionintelligence.org`, and **nginx ranks a leading-wildcard above any regex**, so the app block would always win a plain regex mail block. Fix = make BOTH server_names regexes and rely on definition order (first matching regex wins):
- base mail block: `server_name mail.orionintelligence.org ~^[a-z0-9-]+mail\.orionintelligence\.org$;` (kept on the existing `try.orionintelligence.org` cert), defined **before**
- app block: `server_name try.orionintelligence.org ~^.+\.orionintelligence\.org$;`

**GOTCHA that took the whole platform down (2026-09-23):** in `server_name` the regex marker is **`~`**, and case-insensitivity is `(?i)` inside the pattern — **`~*` is `location`-only syntax**. Writing `~*^…` makes nginx treat `~` as the marker and `*^…` as the pattern, so PCRE rejects the leading `*` (`quantifier does not follow a repeatable item`), nginx refuses to start, and since this file is the box's :443 terminator the **entire origin goes down → Cloudflare 521** (and the mail box's own nginx crash-loops → 502). The Host is already lowercased before server_name matching, so plain `~^…lowercase…$` matches fine; no case flag needed. Always validate a server_name regex change before deploying: `docker run --rm -v "$PWD/nginx/nginx-prod.conf":/etc/nginx/nginx.conf:ro -v <dir-with-try-cert>:/etc/letsencrypt:ro wokalek/nginx-brotli:latest nginx -t`.

So `<slug>mail` peels off to `$mail_backend`; every other subdomain falls through to `$app_backend`. Both on the `try` cert — fine because CF terminates TLS for the browser (Universal SSL) and reaches origin in Full (non-strict) mode, exactly like the tenant Intelligence apps already do. Orion-mail's app nginx `server_name` matches the same regex so it accepts the forwarded Host. The port-80 block is left on the app; CF reaches origin over 443, so it isn't on the mail path. **Caveat:** a tenant slug ending in `mail` (`gmail`, `webmail`) collides with the mail regex — use a `-mail` separator if that ever matters.

### Cleanup / teardown of the old three-level infra (2026-09-23)

Deleted in-repo: the edge `*.mail.orionintelligence.org` server block + its `mail-wildcard` cert reference (this file); the mail app-nginx `*.mail.…` server_name. Cloudflare: the DNS-only `A *.mail.orionintelligence.org → 144.76.157.34` (the old 3-level webmail record) was **deleted 2026-09-23** — but the `MX` and `TXT`(SPF `v=spf1 ip4:144.76.157.34 -all`) on `*.mail.orionintelligence.org` were **kept**: they are the tenant email-sending infra for the `<slug>.mail.…` sending domain, unrelated to the webmail browser host. Verified `intelysyncmail.orionintelligence.org` resolves to CF proxy IPs via the existing proxied `CNAME *.orionintelligence.org → try.orionintelligence.org` (+ Universal SSL), so new tenant webmail hosts need **zero** DNS/cert work. Remaining prod-box step: `certbot delete --cert-name mail-wildcard` (unused now). Deploy is just `git pull && ./run.sh build -p` on both boxes (`command.txt`).

## Backup spec 500s only in the GitHub pipeline (2026-09-26)

`22-backup-restore.cy.ts` failed every CI run with `createInstantBackup` getting a 500 and passed every local run. `admin_routes` is registered before `test_routes`, so the fake `/api/admin/backups/instant` and `/restore` handlers in `test_routes.py` are shadowed and the spec drives the real `BackupManager`. `start_backup` marks the job running and then touches `/app/static/.maintenance`; `restore_backup` also rewrites `/app/static/resource`. `/app/static` is a bind mount of `backend/static`, the container runs as `app` (uid 1000), and on a GitHub runner the checkout belongs to `runner` (uid 1001) with mode 755, so the touch raises `PermissionError` and the global handler returns 500. Locally the tree is owned by uid 1000 (and is group/world writable), so it never shows. The second test 500s for the same reason once the orphaned running job goes stale. Fixed in `.github/workflows/test.yml` by adding `backend/static` to the `chmod -R a+rwX` step that already covers `backend/workspace/resource`, and adding `/app/static` and `/app/backups` to the in-container writability check so a regression fails at stack start instead of three minutes into spec 22. Reproduced by running `start_backup` and `restore_folder` from the `trusted-search-web` image as uid 1000 against a 755 dir owned by uid 1001 (both `PermissionError`), then again after `chmod -R a+rwX` (both succeed). Locally a real backup takes ~12s and a restore ~40s, well inside the spec's 300s maintenance wait.

## Backup spec test 2 found 2 rows after clearing (2026-09-26)

Once the permission fix landed, test 2 (`opens backup visibility after creating a backup`) failed in CI with "Found '2', expected '1'". `clearAllBackups` took a one-shot `$body.find('[data-testid="backup-row"]:visible')` snapshot right after the page rendered. The rows sit in a `@fadeInDashboardItem` tbody that enters at `opacity: 0`, and Cypress's `:visible` (with `experimentalFastVisibility` it is `checkVisibility({opacityProperty: true})`, which also checks ancestors) calls them hidden during the fade, so the snapshot saw 0 rows and deleted nothing. Test 1 leaves 2 backups, test 2 adds one, retention prunes to `MAX_BACKUPS`, and the spec sees 2. In CI test 1 always starts on an empty database, so test 2 is the only place the clear has anything to delete and the only place the race shows; a faster local machine finishes the fade first. `clearAllBackups` now reads the count from `GET /api/admin/backups`, waits with a retrying `:visible` length assertion before each delete, and checks the API is empty at the end. Verified with a scratch Cypress harness (mock page whose rows fade in from opacity 0, fake backups API): the original controller left 2 of 2 backups, the new one cleared both, and an empty list still passes. Cron is not a factor: `init_cronjobs` does not start `backup_loop` when `TESTING_ENABLED=1`. About 90 other specs still use the same one-shot `$body.find('...:visible')` pattern for optional branches; any of them can take the wrong branch the same way if its target is mid-animation.

  ...
  async def __handle_request(self, request):
    ...
    if m_status is False:
      ...
      if m_leak_index and len(m_leak_index["cards_data"]):
        result = await elastic_controller.get_instance().index_leak(m_leak_index)
        m_response_leak, m_data_leak = result
      ...
      if m_generic_index:
        result = await elastic_controller.get_instance().index_general(m_generic_index)
        m_response_generic, m_data_generic = result

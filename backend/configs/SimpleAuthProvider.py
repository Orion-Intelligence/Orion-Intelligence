...
class TokenAuthProvider(AuthProvider):
    async def login(self, username: str = Form(...), password: str = Form(...), remember_me: bool = Form(False), request: Request = None, response: Response = None) -> Response:
        try:
            ...
            if user.role != user_role.ADMIN: raise HTTPException(status_code=403, detail="Not authorized")
            access_token_expires = timedelta(minutes=30)
            access_token, role = await session_manager.get_instance().create_access_token(data={"sub": user.username}, expires_delta=access_token_expires)
            redirect = RedirectResponse(url="/admin/", status_code=HTTP_303_SEE_OTHER)
            redirect.set_cookie(key="access_token", value=access_token, httponly=True, secure=not IS_DEBUG, samesite="lax" if IS_DEBUG else "strict", max_age=1800, path="/")
            return redirect
        except Exception:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")

    async def is_authenticated(self, request: Request) -> bool:
        if request.url.path == "/admin/login": return True
        ...
        try:
            ...
            if role != user_role.ADMIN.value: return False
    ...
    def get_admin_user(self, request: Request) -> AdminUser:
        ...
        return AdminUser(username=user.username if user else "anonymous", photo_url=None)
...
def setup_admin(engine: AIOEngine) -> Admin:
    admin = Admin(engine, title="Admin Panel", auth_provider=TokenAuthProvider(), base_url="/admin/")

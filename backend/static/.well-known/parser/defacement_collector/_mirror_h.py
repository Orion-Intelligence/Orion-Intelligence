        ...
        try:
            ...
            while current_page <= max_pages:
                ...
                links = page.query_selector_all(
                    "td[style='word-break: break-word;white-space: normal;min-width: 300px;'] a")
                collected_links = []
                for link in links:
                ...
                for link in collected_links:
                    ...
                    web_url = self.safe_find(page,
                                             "//td[i[contains(@class, 'mdi-web')]]/following-sibling::td/strong/a",
                                             "href")
                    location = self.safe_find(page,
                                              "//td[i[contains(@class, 'mdi-map-marker')]]/following-sibling::td/strong")
                    server_ip = self.safe_find(page,
                                               "//td[i[contains(@class, 'mdi-mapbox')]]/following-sibling::td/strong/a")
                    web_server = self.safe_find(page,
                                                "//td[i[contains(@class, 'mdi-server')]]/following-sibling::td/strong/a")
                    attacker = self.safe_find(page,
                                              "//td[i[contains(@class, 'mdi-account')]]/following-sibling::td/strong/a")
                    total = self.safe_find(page,
                                           "//td[i[contains(@class, 'mdi-clipboard-plus')]]/following-sibling::td/strong")
                    date = self.safe_find(page,
                                          "//td[i[contains(@class, 'mdi-calendar')]]/following-sibling::td/strong")
                    ...
                    card_data = defacement_model(m_date_of_leak=helper_method.extract_and_convert_date(date),
                                                 m_web_server=[web_server] if web_server else [],
                                                 m_web_url=[web_url] if web_url else [], m_base_url=self.base_url,
                                                 m_network=helper_method.get_network_type(self.base_url),
                                                 m_content=content,
                                                 m_url=link, m_mirror_links=[iframe_url] if iframe_url else [])

                    entity_data = entity_model(m_ip=[server_ip] if server_ip else [],

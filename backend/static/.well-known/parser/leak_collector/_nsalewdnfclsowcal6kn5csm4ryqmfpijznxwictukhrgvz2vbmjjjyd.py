        ...
        try:
            ...
            for index, card in enumerate(cards):
                try:
                    ...
                    url = weblinks[0]
                    is_crawled = int(
                        self.invoke_db(REDIS_COMMANDS.S_GET_INT, CUSTOM_SCRIPT_REDIS_KEYS.URL_PARSED.value + url, 0,
                                       RAW_PATH_CONSTANTS.HREF_TIMEOUT))
                    ref_html = None
                    if is_crawled != -1 and is_crawled < 5:
                        ref_html = helper_method.extract_refhtml(url)
                        if ref_html:
                            self.invoke_db(REDIS_COMMANDS.S_SET_INT, CUSTOM_SCRIPT_REDIS_KEYS.URL_PARSED.value + url,

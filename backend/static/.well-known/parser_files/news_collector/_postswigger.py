    ...
    def parse_leak_data(self, page: Page):
        ...
        for link in all_links:
            try:
                ...
                date_tag = soup.select_one("time[datetime], span.date, abbr.published")
                date_raw = date_tag.get("datetime") if date_tag and date_tag.has_attr("datetime") else None
                if not date_raw and date_tag:
                    date_raw = date_tag.get_text(strip=True)
                if not date_raw:
                    content_text = soup.get_text(separator=" ", strip=True)
                    match = re.search(r"\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{1,2}, \d{4}\b",
                                      content_text)
                    if match:

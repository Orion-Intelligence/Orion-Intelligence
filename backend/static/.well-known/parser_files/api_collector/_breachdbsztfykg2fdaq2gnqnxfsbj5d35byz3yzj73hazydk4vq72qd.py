  ...
  async def parse_leak_data(self, query: Dict[str, str], context: BrowserContext):
    ...
    for search_type, query_value in [("Username", username), ("Email", email)]:
      ...
      try:
        ...
        public_records = [(await span.text_content()).split("-->", 1)[0].strip() for span in spans if
          "-->" in (await span.text_content())]
    ...
    if combined_records:
      ...
      entity_data = entity_model(
        m_scrap_file=self.__class__.__name__, m_email=list(email_list), m_name=", ".join(username_list))

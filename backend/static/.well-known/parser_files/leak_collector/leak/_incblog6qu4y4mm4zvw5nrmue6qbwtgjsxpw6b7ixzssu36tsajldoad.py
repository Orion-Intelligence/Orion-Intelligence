    ...
    try:
      ...
      for index, item in enumerate(titles_and_links, start=1):
        try:
          ...
          try:
            parsed_date = datetime.strptime(date_text, "%d.%m.%Y %H:%M").replace(
              second=0,
              microsecond=0) if date_text else None
          except:
            parsed_date = None
          ...
          entity_data = entity_model(
            m_scrap_file=self.__class__.__name__, m_team="inc ransome", m_company_name=title, m_industry=industry, )

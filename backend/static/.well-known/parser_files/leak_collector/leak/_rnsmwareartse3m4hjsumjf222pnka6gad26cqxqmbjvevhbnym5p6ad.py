                ...
                try:
                    ...
                    if more_info_url:
                        ...
                        all_links = [link.get_attribute('href') for section_id in
                            ['#Negotiat', '#listing-files', '#files'] for link in
                            more_info_page.query_selector_all(f'{section_id} + .section ul a')]

                        all_links += [link.get_attribute('href') for link in
                            more_info_page.query_selector_all('a[href]') if link.get_attribute('href')]
                    ...
                    entity_data = entity_model(
                        m_scrap_file=self.__class__.__name__, m_company_name=title, m_team="run some wares")

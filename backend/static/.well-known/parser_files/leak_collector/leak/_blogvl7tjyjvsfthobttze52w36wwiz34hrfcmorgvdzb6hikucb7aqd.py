    ...
    def parse_leak_data(self, page: Page):
        ...
        for i in range(1, 26):
            try:
                ...
                title_tag = page.query_selector(
                    'h5.MuiTypography-root.MuiTypography-h5.MuiTypography-alignCenter.css-1pakh3q')
                title = title_tag.inner_text().strip() if title_tag else "No Title"

                date_tag = page.query_selector(
                    'p.MuiTypography-root.MuiTypography-body1.MuiTypography-alignCenter.css-1oy63y8')
                ...
                if publication_date_raw:
                    try:
                        publication_date = datetime.strptime(
                            publication_date_raw.split(':')[-1].strip(),
                            '%d.%m.%Y').date()
                    except Exception:
                        pass
                ...
                ref_html = helper_method.extract_refhtml(
                    weblinks[0] if weblinks else title,
                    self.invoke_db,
                    REDIS_COMMANDS,
                ...
                entity_data = entity_model(
                    m_scrap_file=self.__class__.__name__, m_company_name=title, m_team="money message")

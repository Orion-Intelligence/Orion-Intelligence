    ...
    for index in range(count):
      ...
      try:
        ...
        size_match = re.search(
          r"\b(\d+(?:\.\d+)?\s?(?:KB|MB|GB|TB|PB|KiB|MiB|GiB|TiB))\b",
          card_text,
          flags=re.IGNORECASE)
        ...
        title_url = helper_method.clean_text(
          title_element.text_content().strip()) if title_element.count() else card_title_url
        ...
        raw_lines = [page.evaluate(
          "(html) => { const div = document.createElement('div'); div.innerHTML = html; return div.textContent; }",
        ...
        dump_links = links_element.locator("a[href]").evaluate_all(
          "els => els.map(el => el.href)") if links_element.count() else []
        ...
        entity_data = entity_model(
          m_scrap_file=self.__class__.__name__, m_company_name=title_name if title_name else None, m_team="abyss")

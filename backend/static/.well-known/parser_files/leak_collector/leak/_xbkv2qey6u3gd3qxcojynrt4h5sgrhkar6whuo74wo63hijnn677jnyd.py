                ...
                try:
                    for link, main_page_data_size in collected_links:
                        try:
                            ...
                            m_revenue = revenue_element.inner_text().replace(
                                "Revenue:", "").strip() if revenue_element else ""

                            size_element = page.query_selector("article > p:nth-child(4)")
                            m_data_size = size_element.inner_text().replace(
                                "Data:", "").strip() if size_element else main_page_data_size
                            ...
                            entity_data = entity_model(
                                m_scrap_file=self.__class__.__name__, m_team="unknown")

  ...
  def parse_leak_data(self, page: Page):
    ...
    for victim_url in victim_links:
      try:
        ...
        content_lines = [para.inner_text().strip() for para in paragraphs if para.inner_text().strip() and not any(
        ...
        if victim_name and " " not in victim_name:
          try:
            ref_html = helper_method.extract_refhtml(
              victim_name, self.invoke_db, REDIS_COMMANDS, CUSTOM_SCRIPT_REDIS_KEYS, RAW_PATH_CONSTANTS, page)

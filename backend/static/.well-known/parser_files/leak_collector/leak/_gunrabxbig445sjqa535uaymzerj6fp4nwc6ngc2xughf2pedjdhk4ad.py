  ...
  def parse_leak_data(self, page: Page):
    try:
      ...
      for block_index, block in enumerate(company_blocks, start=1):
        ...
        title_links.append(
          {"block_index": block_index, "title": title, "industry": industry, "location": location, "publish_date": None, "weblink": weblink, "dumplink": dumplink, "description": description})

      for title_data in title_links:
        ref_html = helper_method.extract_refhtml(
          title_data["weblink"], self.invoke_db, REDIS_COMMANDS, CUSTOM_SCRIPT_REDIS_KEYS, RAW_PATH_CONSTANTS, page)

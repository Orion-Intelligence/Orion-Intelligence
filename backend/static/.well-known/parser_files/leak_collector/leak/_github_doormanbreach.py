...
class _github_doormanbreach(leak_extractor_interface, ABC):
  ...
  def parse_leak_data(self, page: Page):
    ...
    hrefs = page.eval_on_selector_all(
      'a.Link--primary', 'els => els.map(e => e.getAttribute("href"))')
    ...
    for idx, md_url in enumerate(md_links, start=1):
      ...
      if bd_h2:
        ...
        for sibling in bd_h2.find_parent().find_next_siblings():
          if sibling.name and sibling.name.startswith('h'):
            break
          txt = (sibling.get_text(strip=True) if sibling.name == 'p' else ", ".join(
            li.get_text(strip=True) for li in sibling.find_all('li')) if sibling.name == 'ul' else "")
      ...
      if download_h2:
        ...
        candidates += [a['href'] for a in download_h2.find_all_next('a', href=True) if
          a.find_previous('h2') is download_h2]
        download_links = list(
          dict.fromkeys(
      ...
      entity_data = entity_model(
        m_scrap_file=self.__class__.__name__, m_team="doormanBreach", )

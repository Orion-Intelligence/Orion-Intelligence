from abc import ABC
from typing import List
import re
from playwright.sync_api import Page

from crawler.constants.constant import RAW_PATH_CONSTANTS
from crawler.crawler_instance.local_interface_model.leak.leak_extractor_interface import leak_extractor_interface
from crawler.crawler_instance.local_shared_model.data_model.entity_model import entity_model
from crawler.crawler_instance.local_shared_model.data_model.leak_model import leak_model
from crawler.crawler_instance.local_shared_model.rule_model import RuleModel, FetchProxy, FetchConfig, ThreatType
from crawler.crawler_services.redis_manager.redis_controller import redis_controller
from crawler.crawler_services.redis_manager.redis_enums import REDIS_COMMANDS, CUSTOM_SCRIPT_REDIS_KEYS
from crawler.crawler_services.shared.helper_method import helper_method


class _fjg4zi4opkxkvdz7mvwp7h6goe4tcby3hhkrz43pht4j3vakhy75znyd(leak_extractor_interface, ABC):
    _instance = None

    def __init__(self, callback=None):

        self.callback = callback
        self._card_data = []
        self._entity_data = []
        self.soup = None
        self._initialized = None
        self._redis_instance = redis_controller()
        self._is_crawled = False

    def init_callback(self, callback=None):

        self.callback = callback

    def __new__(cls, callback=None):

        if cls._instance is None:
            cls._instance = super(_fjg4zi4opkxkvdz7mvwp7h6goe4tcby3hhkrz43pht4j3vakhy75znyd, cls).__new__(cls)
            cls._instance._initialized = False
        return cls._instance

    @property
    def is_crawled(self) -> bool:
        return self._is_crawled

    @property
    def seed_url(self) -> str:
        return "http://fjg4zi4opkxkvdz7mvwp7h6goe4tcby3hhkrz43pht4j3vakhy75znyd.onion/#files-heading"

    @property
    def developer_signature(self) -> str:
        return "Usman Ali:qDMP"


    @property
    def base_url(self) -> str:
        return "http://fjg4zi4opkxkvdz7mvwp7h6goe4tcby3hhkrz43pht4j3vakhy75znyd.onion"

    @property
    def rule_config(self) -> RuleModel:
        return RuleModel(m_fetch_proxy=FetchProxy.TOR, m_fetch_config=FetchConfig.PLAYRIGHT, m_resoource_block=False,m_threat_type= ThreatType.LEAK)

    @property
    def card_data(self) -> List[leak_model]:
        return self._card_data

    @property
    def entity_data(self) -> List[entity_model]:
        return self._entity_data

    def invoke_db(self, command: int, key: str, default_value, expiry: int = None):

        return self._redis_instance.invoke_trigger(command, [key + self.__class__.__name__, default_value, expiry])

    def contact_page(self) -> str:
        return "http://fjg4zi4opkxkvdz7mvwp7h6goe4tcby3hhkrz43pht4j3vakhy75znyd.onion/web/contacts"

    def append_leak_data(self, leak: leak_model, entity: entity_model):

        self._card_data.append(leak)
        self._entity_data.append(entity)
        if self.callback:
            if self.callback():
                self._card_data.clear()
                self._entity_data.clear()

    def parse_leak_data(self, page: Page):
        page.goto(self.seed_url, wait_until="domcontentloaded", timeout=20000)

        ack = page.locator("label.ann-modal-ack")
        if ack.count() > 0:
            ack.click()

        page.wait_for_selector("article.company-row", timeout=15000)
        cards = page.locator("article.company-row").all()
        article_detail_urls = []

        for card in cards:
            href = card.locator('a.btn.btn-primary').get_attribute('href')
            if not href:
                continue
            full_detail_url = self.base_url + href if href.startswith('/') else href
            article_detail_urls.append(full_detail_url)

        for detail_url in article_detail_urls:
            page.goto(detail_url, wait_until="domcontentloaded", timeout=20000)

            title_elem = page.locator('h1')
            title = (
                title_elem.inner_text().replace('\n', '').strip()
                if title_elem.count() > 0
                else detail_url.split('/')[-1].replace('-', ' ').title()
            )

            meta_items = page.locator('div.meta-row div.meta-item').all()
            industry = ""
            revenue = ""
            website = ""
            for item in meta_items:
                label = item.locator('div.meta-label').inner_text().strip()
                value = item.locator('div.meta-value').inner_text().strip()
                if label == "Industry":
                    industry = value
                elif label == "Revenue":
                    revenue = value
                elif label == "Website":
                    link = item.locator('div.meta-value a')
                    if link.count() > 0:
                        website = link.get_attribute('href') or ""
                    else:
                        website = item.locator('div.meta-value').inner_text().strip()
                        if website.lower() == "not provided":
                            website = ""

            overview_locator = page.locator(
                'div.box:has(h2.box-title:text("Company Overview")) div.box-content'
            )
            overview = overview_locator.inner_text().strip() if overview_locator.count() > 0 else ""

            disclosed_data_elem = page.locator(
                'div.box:has(h2.box-title:text("Disclosed Data")) div.box-content'
            )
            disclosed_data_text = disclosed_data_elem.inner_text().strip() if disclosed_data_elem.count() > 0 else ""

            url_match = re.match(r'(https?://[^\s]+)', disclosed_data_text)
            if url_match:
                disclosed_url = url_match.group(1).strip()
                disclosed_description = disclosed_data_text[len(disclosed_url):].strip()
            else:
                disclosed_url = disclosed_data_text
                disclosed_description = ""

            if disclosed_description:
                overview += " " + disclosed_description

            m_content = (
                f"Title: {title}\n"
                f"Industry: {industry}\n"
                f"Revenue: {revenue}\n"
                f"Website: {website}\n"
                f"Overview: {overview}\n"
                f"Disclosed Data URL: {disclosed_url}\n"
                f"Disclosed Data Description: {disclosed_description}"
            )

            ref_html = helper_method.extract_refhtml(
                website,
                self.invoke_db,
                REDIS_COMMANDS,
                CUSTOM_SCRIPT_REDIS_KEYS,
                RAW_PATH_CONSTANTS,
                page
            )

            card_data = leak_model(
                m_title=title,
                m_ref_html=ref_html,
                m_url=detail_url,
                m_base_url=self.base_url,
                m_content=m_content,
                m_network=helper_method.get_network_type(self.base_url),
                m_screenshot=helper_method.get_screenshot_base64(page, title, self.base_url),
                m_important_content=overview,
                m_weblink=[website] if website else [],
                m_content_type=["leaks"],
                m_dumplink=[disclosed_url] if disclosed_url else [],
                m_revenue=revenue
            )

            entity_data = entity_model(
                m_team="Coinbase Cartel",
                m_industry=industry,
                m_scrap_file=self.__class__.__name__,
            )

            self.append_leak_data(card_data, entity_data)
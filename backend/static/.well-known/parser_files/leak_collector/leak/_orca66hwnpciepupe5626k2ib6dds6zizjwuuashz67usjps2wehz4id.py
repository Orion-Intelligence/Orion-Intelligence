...
class _orca66hwnpciepupe5626k2ib6dds6zizjwuuashz67usjps2wehz4id(leak_extractor_interface, ABC):
  ...
  def parse_leak_data(self, page: Page):
    try:
      ...
      card_urls = [urljoin(self.base_url, link.get_attribute("href")) for link in card_links if
      ...
      for card_url in card_urls:
        try:
          ...
          date_text = next(
            (item.query_selector("p.card__info-text").text_content().strip() for item in
            page.query_selector_all("div.card__info-item") if "date of publication" in (
              item.query_selector("h2").text_content().strip().lower() if item.query_selector("h2") else "")), None)
          if date_text:
          ...
          entity_data = entity_model(
            m_scrap_file=self.__class__.__name__, m_company_name=card_title, m_team="public ocra")

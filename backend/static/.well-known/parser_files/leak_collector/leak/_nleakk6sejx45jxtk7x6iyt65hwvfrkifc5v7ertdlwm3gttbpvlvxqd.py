...
class _nleakk6sejx45jxtk7x6iyt65hwvfrkifc5v7ertdlwm3gttbpvlvxqd(leak_extractor_interface, ABC):
  ...
  def parse_leak_data(self, page: Page):
    ...
    view_links = page.eval_on_selector_all(
      "div.card a.btn.btn-secondary", "elements => elements.map(el => el.href)")
    ...
    for link in view_links:
      ...
      country = page.locator("[data-ref='subject_country']").inner_text() if page.locator(
        "[data-ref='subject_country']").count() > 0 else ""

      publish_date_text = page.locator("[data-ref='article_published_timestamp']").inner_text() if page.locator(
        "[data-ref='article_published_timestamp']").count() > 0 else ""
      ...
      weblink = page.locator("[data-ref='subject_site'] a").get_attribute("href") if page.locator(
        "[data-ref='subject_site'] a").count() > 0 else ""

      attached_file_link = page.locator("[data-ref='download_link']").get_attribute("href") if page.locator(
        "[data-ref='download_link']").count() > 0 else ""
      ...
      entity_data = entity_model(
        m_scrap_file=self.__class__.__name__, m_team="LEAKNET BLOG", m_country=[country], )

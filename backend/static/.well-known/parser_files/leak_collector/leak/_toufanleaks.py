...
class _toufanleaks(leak_extractor_interface, ABC):
  ...
  def parse_leak_data(self, page: Page):
    ...
    links = page.eval_on_selector_all(
      "h5.entry-title a", "elements => elements.map(el => el.href)")
    ...
    for link in links:
      try:
        ...
        images = page.eval_on_selector_all(
          "div.metadata img", "els => els.map(e => e.src)")

        content_texts = page.eval_on_selector_all(
          "div.metadata p, pre.wp-block-preformatted", "els => els.map(e => e.innerText)")
        ...
        entity_data = entity_model(
          m_team="cyber_toufan", m_scrap_file=self.__class__.__name__, )

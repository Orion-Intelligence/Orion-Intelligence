        ...
        for _ in range(max_pages):
            ...
            try:
                ...
                card_data = leak_model(
                    ...
                    m_leak_date=next(
                        (datetime.strptime(match.group(1), "%Y-%m-%d").strftime("%Y-%m-%d") for value in
                        data_dict.values() if isinstance(value, str) and (
                            match := re.search(r"Breach date[:：]?\s*(\d{4}-\d{2}-\d{2})", value))), None, ),
                    m_data_size=f"{data_dict['Total Affected']} individuals" if "Total Affected" in data_dict and
                                                                                data_dict[

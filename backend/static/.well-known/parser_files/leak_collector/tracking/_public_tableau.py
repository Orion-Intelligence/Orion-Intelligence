  ...
  def parse_leak_data(self, page: Page):
    ...
    for _ in range(max_pages):
      ...
      try:
        ...
        content_parts = [data_dict.get("Incident Details", ""), data_dict.get("Breach Type", ""),
          data_dict.get("Organization Type", ""), data_dict.get("Information Impacted", "")]
        ...
        card_data = leak_model(
          ...
          m_leak_date=next(
            (datetime.strptime(match.group(1), "%Y-%m-%d").strftime("%Y-%m-%d") for value in data_dict.values() if
            isinstance(value, str) and (match := re.search(r"Breach date[:：]?\s*(\d{4}-\d{2}-\d{2})", value))), None, ),
          m_data_size=f"{data_dict['Total Affected']} individuals" if "Total Affected" in data_dict and data_dict[
            "Total Affected"] != "UNKN" else None, )

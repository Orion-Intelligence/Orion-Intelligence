...
class _mcf(leak_extractor_interface, ABC):
    _instance = None
    ...
    def __new__(cls, callback=None):
        if cls._instance is None:
            cls._instance = super(_mcf, cls).__new__(cls)
    ...
    def seed_url(self) -> str:
        return "https://www.mcf.se/sv/aktuellt/nyheter/?sortOrder=DescendingYear"
    ...
    def contact_page(self) -> str:
        return "https://www.mcf.se/sv/om-oss/kontakta-oss/"
    ...
    def parse_leak_data(self, page: Page):
        try:
            ...
            has_next = True
            while has_next:
                news_cards = page.query_selector_all("div.news-list-card-wrapper")
                ...
                for card in news_cards:
                    ...
                    info_div = card.query_selector("p.news-list-card-date")
                    date_str = ""
                    if info_div:
                    ...
                    list_summary = ""
                    summary_el = card.query_selector("p.news-list-card-content")
                    if summary_el:
                        list_summary = summary_el.inner_text().strip()
                    ...
                    if list_summary:
                        content_parts.append(list_summary)


                    main_body_div = detail_page.query_selector("div.page-main-body")
                    if not main_body_div:
                        main_body_div = detail_page.query_selector("article")
                    if not main_body_div:
                        main_body_div = detail_page.query_selector("main")
                    ...
                    link_container = main_body_div if main_body_div else detail_page
                    for a in link_container.query_selector_all("a[href]"):
                ...
                if next_btn:
                    ...
                    # Check for valid link that isn't javascript or empty
                    if next_href and not next_href.startswith("javascript") and next_href != "#":

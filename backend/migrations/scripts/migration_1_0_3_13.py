...
class migration_1_0_3_13:
    ...
    async def migrate(version):
        ...
        await migration_1_0_3_13.backfill_tenant_slugs(engine)
        await migration_1_0_3_13.migrate_system_settings(engine, default_tenant, version)
        await migration_1_0_3_13.ensure_indexes(engine)
    ...
    async def migrate_system_settings(engine, default_tenant, version):
        ...
        async for doc in settings_collection.find({}):
            ...
            if key == AllowedKeys.SYSTEM_SETTINGS.value:
                settings_by_tenant.setdefault(tenant_id, {}).update(
                    migration_1_0_3_13._loads(doc.get("value"))
                )
            else:
        ...
        await migration_1_0_3_13._drop_old_system_indexes(settings_collection)
        ...
        for tenant_doc in tenants:
            ...
            meta_info = migration_1_0_3_13._loads(settings.get(AllowedKeys.META_INFO.value))
    ...
    async def ensure_indexes(engine):
        settings_collection = engine.get_collection(db_system_model)
        await migration_1_0_3_13._drop_old_system_indexes(settings_collection)
        if not await migration_1_0_3_13._has_index(settings_collection, [("tenant_id", 1)], unique=True):
        ...
        tenant_collection = engine.get_collection(db_tenant_model)
        if not await migration_1_0_3_13._has_index(
            tenant_collection,
            [("slug", 1)],
            unique=True,
            sparse=True,

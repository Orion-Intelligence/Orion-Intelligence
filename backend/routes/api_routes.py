...
@api_routes.get("/api/directory", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST]))],
...
@api_routes.get("/api/dumps", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("module:dumps"))],
                description="Fetch the directory listing with optional filters for categories, types, or tags.")
async def get_directory(param: dump_param_model = Depends()):
    return await dump_model.getInstance().invoke_dump(param)


@api_routes.get("/api/insight", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST]))],
...
@api_routes.post("/api/nlp/chat/report", dependencies=[Depends(role_required([user_role.ADMIN])), Depends(limiter_dependency)])
async def chat_report(payload: ReportChatRequest):
    response = await crawl_model.getInstance().parse_chat_ai(payload)
    return response


@api_routes.post("/api/search/strategic", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("module:general"))],
...
@api_routes.post("/api/search/stealerlogs", dependencies=[Depends(role_required([user_role.ADMIN, user_role.PROFILE, user_role.DEMO,user_role.ANALYST])),Depends(license_required("module:stealer_logs"))])
...
@api_routes.post("/api/search/consolidated", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("maintainer",[user_role.ADMIN,user_role.ANALYST]))],
...
@api_routes.post("/api/search/consolidated/ranked", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("maintainer",[user_role.ADMIN,user_role.ANALYST]))],
                 description="Search breach (leak) intelligence reports using parameters such as company, country, or hash.")
async def search_consolidated(param: search_consolidated_param_model = Body(...)):
    base_index = [
...
@api_routes.post("/api/chat/telegram", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("module:social"))])
...
@api_routes.post("/api/search/discussion", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("module:discussion"))],
...
@api_routes.post("/api/exploit/discussion", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("module:exploit"))])
...
@api_routes.post("/api/social/all", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])), Depends(license_required("module:social"))])
async def search_discussion(param: search_general_param_model = Body(...)):
    base_index = [
        ELASTIC_INDEX.S_CHATS_INDEX,
        ELASTIC_INDEX.S_SOCIAL_INDEX
...
@api_routes.post("/api/social", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])), Depends(license_required("module:social"))])
...
@api_routes.post("/api/search/breach", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("module:breach"))],
                 description="Search breach (leak) intelligence reports using parameters such as company, country, or hash.")
async def search_leak(param: search_leak_param_model = Body(...)):
    if param.category in ['all']:
        base_index = [
...
@api_routes.post("/api/search/news", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST]))],
...
@api_routes.post("/api/search/exploit", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("module:exploit"))],
                 description="Search breach (leak) intelligence reports using parameters such as company, country, or hash.")
async def search_leak(param: search_leak_param_model = Body(...)):
    return await search_model.getInstance().search_exploit_result(param)


@api_routes.post("/api/search/defacement", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])),Depends(license_required("module:defacement"))],
...
@api_routes.get("/api/search/breach/{doc_id}", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST]))],
...
@api_routes.get("/api/search/news/{doc_id}", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST]))],
                description="Get a specific breach (leak) document by its document ID and optional language.")
async def get_leak_document(doc_id: str, lang: Optional[str] = Query(None, alias="lang")):
    return await search_model.getInstance().request_leak_doc(doc_id, lang)
...
@api_routes.get("/api/search/chat/{doc_id}", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST]))],
                description="Get a specific strategic report document by its document ID and optional language.")
async def get_general_document(doc_id: str, lang: Optional[str] = Query(None, alias="lang")):
    return await search_model.getInstance().request_chat_doc(doc_id, lang)


@api_routes.get("/api/search/social/{doc_id}", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST]))],
                description="Get a specific strategic report document by its document ID and optional language.")
...
@api_routes.post("/api/dynamic/user", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])), Depends(license_required("scanning"))],
...
@api_routes.post("/api/dynamic/cracked", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])), Depends(license_required("scanning"))],
                 description="Perform a dynamic search for emails found in breach and defacement data.")
async def search_dynamic_email(param: search_dynamic_param_model = Body(...)):
    return await search_model.getInstance().dynamic_search(param, "cracked")

@api_routes.post("/api/urlscan/domain", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])), Depends(limiter_dependency),Depends(license_required("scanning"))])
async def parse_text(payload: DomainScanRequest):
    return await crawl_model.getInstance().scan_domain(payload)

@api_routes.post("/api/dynamic/social", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])), Depends(license_required("scanning"))],
                 description="Perform a dynamic search for emails found in breach and defacement data.")
async def search_dynamic_email(param: search_dynamic_param_model = Body(...)):
    return await search_model.getInstance().dynamic_search(param, "social")


@api_routes.get("/api/search/breach/screenshot/{filename}", dependencies=[Depends(role_required([user_role.ADMIN, user_role.DEMO, user_role.PROFILE,user_role.ANALYST])), Depends(license_required("module:breach"))],
...
@api_routes.post("/api/get/company/profile", dependencies=[Depends(role_required([user_role.PROFILE])), Depends(status_required([UserStatus.ACTIVE])),Depends(license_required("maintainer"))])
...
@api_routes.post("/api/update/company/profile", dependencies=[Depends(role_required([user_role.PROFILE])), Depends(status_required([UserStatus.ACTIVE])),Depends(license_required("maintainer"))])
...
@api_routes.post("/api/upload/image", dependencies=[Depends(role_required([user_role.PROFILE])), Depends(status_required([UserStatus.ACTIVE])),Depends(license_required("maintainer"))])

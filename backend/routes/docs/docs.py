...
SEARCH_DOCS = {
    ...
    "news": {
        "description": (
            "Search breach-related news articles aggregated from OSINT sources, ransomware/extortion blogs, "
            "darkweb portals, and other cyber-security news feeds; returns a paginated list of news items linked "
            "to breaches and leaks.\n\n"
            "This endpoint reuses the same request model as `/api/search/breach` (`search_leak_param_model`) "
            "but **always restricts results to news content** by setting the internal field "
            "Request body (`search_leak_param_model`):\n"
            "- **q** — free-text search term applied across headline, body, victim name, group name, domains, etc. "
            "(default: empty string).\n"
            "- **category** — logical content bucket. `\"all\"` lets the backend decide the best news collections; "
            "other values may restrict to specific internal sets when configured (default: `\"all\"`).\n"
            ...
            "when `false`, all matching news items are considered (frontend typically maps `\"yes\"/\"no\"` to this boolean).\n"
            "- **profile** — when `true`, enables profile-based boosting/filters (for example, prioritising news about "
            "watched entities) if such logic is configured.\n"
            "- **network** — web layer filter; one of:\n"
            "  - `\"all\"` — no restriction\n"
            "  - `\"clearnet\"` — public news sites\n"
            "  - `\"onion\"` — onion/ransomware blogs mirrored as news\n"
            "  - `\"i2p\"` — I2P news sources (if any)\n"
            "- **matchtype** — logical operator for combining the main query and filters; usually `\"or\"` (default) "
            "or `\"and\"`.\n"
            "- **daterange** — optional creation/ingestion date range for the news document in "
            "`YYYY-MM-DD,YYYY-MM-DD` format; empty string means no date filter.\n"
            "- **content** — content-type selector used when the underlying leak index also stores MITRE/STIX objects "
            "(for example, `\"attack-pattern\"`, `\"malware\"`). For pure news usage, `\"all\"` (default) is typical.\n"
            "- **entity** — optional entity identifier (for example, internal ID, main domain, or organisation key) "
            "to restrict news to that entity when supported by the backend.\n"
            "- **must** — when `true`, values supplied in `entity_filter` are treated as mandatory (`must`) filters; "
            "when `false` they may be treated as soft (`should`) filters depending on configuration.\n"
            "- **entity_filter** — IOC-style structured filter map of `field_name → [values]` to narrow down news items "
            "by specific attributes. Example valid payload:\n"
            "```json\n"
            "{\n"
            "  \"entity_filter\": {\n"
            "    \"m_country\": [\"Germany\"],\n"
            ...
            "}\n"
            "```\n"
            "Commonly used filter keys include:\n"
            "- `m_country` — affected country/countries\n"
            "- `m_location` — location/region strings\n"
            "- `m_team` — group / ransomware gang name\n"
            "- `m_industry` — victim industry vertical\n"
            "- `m_domain` — victim or primary domains mentioned in the article\n"
            "- `m_network` — network type (`\"clearnet\"`, `\"onion\"`, `\"i2p\"`)\n"
            "- `m_content_type` / `content_type` — internal classification tags (for this endpoint, will include `\"news\"`)\n"
            "- `m_language` — article language codes\n"
            "- `m_person` — extracted people or organisation names\n"
            "- any other indexed IOC-style fields exposed by the underlying news/leak index.\n\n"
            "Minimal example request:\n"
            "```json\n"
            "{\n"
            "  \"q\": \"Germany data breach\",\n"
            "  \"category\": \"all\",\n"
            "  \"page\": 1,\n"
            "  \"network\": \"clearnet\",\n"
            "  \"safe\": true,\n"
            "  \"daterange\": \"2025-11-01,2025-12-05\",\n"
            "  \"entity_filter\": {\n"
            "    \"m_country\": [\"Germany\"]\n"
            "  },\n"
            "  \"matchtype\": \"or\"\n"
            "}\n"
            "```\n"
            "All parameters behave the same way as in `/api/search/breach`; the only enforced difference is that this "
            "route always searches documents whose content type is `\"news\"`."
        ),
        "response_description": (
            "Breach news search results as a JSON object describing matching news articles and posts.\n\n"
            "Top-level response keys:\n"
            "- **Result** — list of news report objects\n"
            "- **Suggestions** — optional list of suggested queries or corrections (may be empty or omitted)\n"
            "- **Page_Count** — number of pages for the current query and filters (may be fractional depending on "
            "backend pagination strategy)\n\n"
            "Each entry in **Result** typically contains a subset of the following fields:\n"
            "- **m_title** — news headline (for example `\"Agricultural sector breach in Germany\"`)\n"
            "- **m_url** — URL of the news item (article/post page)\n"
            "- **m_base_url** — base/source URL of the news site or feed\n"
            "- **m_network** — network type such as `\"clearnet\"` or `\"onion\"`\n"
            "- **m_content_type** — classification tags; for this endpoint it will always include `\"news\"`, and may "
            "also include other internal tags such as `\"darkweb\"`, `\"ransomware\"`, etc.\n"
            "- **m_content** — full article body or combined summary text\n"
            "- **m_important_content** — condensed / highlighted version of the article content used for ranking\n"
            "- **m_industry** — victim’s industry vertical, when extracted\n"
            "- **m_location** — list of locations/regions mentioned\n"
            "- **m_country** — list of affected countries\n"
            "- **m_team** — threat group or ransomware gang associated with the news, when identified\n"
            "- **m_domain** — domains associated with the article (victim domain, leak portal, etc.)\n"
            "- **m_websites** — list of victim websites referenced in the news item\n"
            "- **m_logo_or_images** — list of key images or logos referenced in the article\n"
            "- **m_person** — people or organisations referenced, when available\n"
            "- **m_language** — language(s) of the article content\n"
            "- **m_scrap_file** — internal scraper/source identifier\n"
            "- **m_hash** — stable internal hash identifier for the document used for deduplication\n"
            "- **m_update_date** — last update timestamp of the news record\n"
            "- **m_creation_date** — first ingestion timestamp of the news record\n"
            "- **rank_index** — internal ranking/index identifier\n"
            ...
            "  \"Result\": [\n"
            "    {\n"
            "      \"m_title\": \"Ingenieurb\\u00fcro Laudi ransomware leak reported\",\n"
            "      \"m_url\": \"https://example.com/news/ingenieurburo-laudi-breach\",\n"
    ...
    "breach": {
        "description": (
            ...
            "- **profile** — when `true`, enables profile-based boosting/filters (for example, prioritising leaks related "
            "to saved entities or watchlists) if configured.\n"
            "- **network** — web layer filter; one of:\n"
            "  - `\"all\"` — no restriction\n"
            ...
            "- **content** — content-type selector, primarily used for MITRE / STIX-style objects when the backend "
            "is configured for TTP or STIX collections. Typical values include:\n"
            "  - `\"attack-pattern\"`\n"
            "  - `\"campaign\"`\n"
            "  - `\"course-of-action\"`\n"
            "  - `\"identity\"`\n"
            ...
            "  - `\"x-mitre-matrix\"`\n"
            "  - `\"x-mitre-tactic\"`\n"
            "  Use `\"all\"` (default) to disable this filter.\n"
            "- **entity** — optional entity identifier (for example, an internal ID, domain, or organisation key) "
            "to restrict the search to breaches linked to that entity when supported by the backend.\n"
            "- **must** — when `true`, values supplied in `entity_filter` are treated as mandatory (`must`) filters; "
            "when `false` they may be treated as soft (`should`) filters depending on configuration.\n"
            ...
            "}\n"
            "```\n"
            "Example using MITRE / STIX content selector:\n"
            "```json\n"
            "{\n"
            "  \"q\": \"phishing\",\n"

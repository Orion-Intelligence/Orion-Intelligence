...
async def nexus_chat(payload: ReportChatRequest, current_user=Depends(get_current_user)):
    response = await nexus_manager.getInstance().parse_chat(payload, user_id=str(current_user.id), stream=True)
    return response
...
async def clear_nexus_chat_session(current_user=Depends(get_current_user)):
    return await nexus_manager.getInstance().clear_chat_session(user_id=str(current_user.id))

...
class elastic_request_generator:
    ...
    def on_search_general_data(p_query_model, pfilter=None):
        ...
        if len(raw_query)>2 and raw_query != "*" and p_query_model.matchtype=="semantic" and env_handler.get_instance().env("SEMANTIC_ENABLED") == "1":
            try:
                qvec = elastic_semantic_controller.get_instance().embed_query_sync(p_query_model.q)
                if qvec:
                    knn_clause = {
                    ...
                    query_statement["query"].append(knn_clause)
            except Exception:
                pass

...
class elastic_request_generator:
  ...
  def _build_query_block(p_query_model,
      ...
      if quoted_value:
        ...
        for phrase in exact_phrases:
          content_query["bool"]["should"].append(
            {"bool": {"should": [{"match_phrase": {field: {"query": phrase, "boost": boost}}} for field, boost in
              phrase_fields], "minimum_should_match": 1}})
      else:
        for phrase in exact_phrases:
          must_clauses.append(
            {"bool": {"should": [{"match_phrase": {field: {"query": phrase, "boost": boost}}} for field, boost in
    ...
    base_bool_query = {"must": [content_query] if isinstance(
      content_query,
      dict) else [], "filter": must_clauses + must_filter_clauses, "must_not": must_not_clause, }
  ...
  def on_bulk_domain_lookup(p_query_model, pFilter=None):
    ...
    if pFilter:
      ...
      if "m_search_all" in pFilter:
        domains.extend(
          [v for v in pFilter["m_search_all"] if re.search(r'(https?://|[a-z0-9.-]+\.[a-z]{2,})', str(v), re.I)])
    ...
    if p_query_model.daterange:
      ...
      if len(parts) == 2:
        try:
          ...
          must_clauses.append(
            {"range": {"m_leak_date": {"gte": from_date_obj.strftime("%Y-%m-%d"), "lte": to_date_obj.strftime(
              "%Y-%m-%d")}}})
        except ValueError:
          pass
  ...
  def on_search_telegram_data(p_query_model, pfilter=None):
    ...
    query = {"min_score": 0, "query": {"function_score": {"query": {"bool": {"must": [query_string_query] if isinstance(
      query_string_query,
      dict) else [], "filter": (must_clauses + must_filter_clauses + (
      [{"bool": {"should": should_filter_clauses.get("bool", {}).get(
        "should",
        []), "minimum_should_match": 1}}] if not getattr(
        p_query_model,
        "must",
      ...
      "<em>"], "post_tags": ["</em>"]}, "m_caption": {"fragment_size": 250, "number_of_fragments": 3, "pre_tags": [
      "<em>"], "post_tags": ["</em>"]}, "m_ref_html": {"fragment_size": 250, "number_of_fragments": 3, "pre_tags": [
      "<em>"], "post_tags": [
  ...
  def index_query_credential(p_index_data):
    ...
    for credential in p_index_data.get("m_credential_data", []):
      ...
      doc = {k: [i for i in v if i not in (None, "", "null")] if isinstance(v, list) else v for k, v in doc.items() if
        v not in (None, "", "null") and (not isinstance(v, list) or [i for i in v if i not in (None, "", "null")])}

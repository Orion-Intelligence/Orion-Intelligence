...
class elastic_stats_consolidated:
    @staticmethod
    ...
    def on_insight_consolidated_data(p_query_model):
        ...
        i1, q1 = elastic_stats_consolidated.on_insight_leakdata(m1)
        queries.append(elastic_stats_consolidated._strip_query(q1))
        indices.append(i1)

        m2 = clone_model(p_query_model)
        i2, q2 = elastic_stats_consolidated.on_insight_general_data(m2)
        queries.append(elastic_stats_consolidated._strip_query(q2))
        indices.append(i2)

        m3 = clone_model(p_query_model)
        i3, q3 = elastic_stats_consolidated.on_insight_exploitdata(m3)
        queries.append(elastic_stats_consolidated._strip_query(q3))
        indices.append(i3)

        m4 = clone_model(p_query_model)
        i4, q4 = elastic_stats_consolidated.on_insight_telegram_data(m4)
        queries.append(elastic_stats_consolidated._strip_query(q4))
        indices.append(i4)

        m5 = clone_model(p_query_model)
        i5, q5 = elastic_stats_consolidated.on_insight_defacement_data(m5)
        queries.append(elastic_stats_consolidated._strip_query(q5))
        indices.append(i5)

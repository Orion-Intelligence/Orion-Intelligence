...
class StixManager:
  ...
  def _convert_defacement(self, raw: DefacementResultItem) -> Dict[str, Any]:
    ...
    if infra_seed:
      infra = {"type": "infrastructure", "spec_version": "2.1", "id": stix_id(
        ...
        "unknown"], "first_seen": created, "last_seen": modified, "labels": sorted(labels), "object_marking_refs": [
        tlp_amber_id], "x_orion_network": str(network) if network else None, }
    ...
    tactics = [str(x).strip().lower().replace(" ", "-") for x in as_list(safe_get(raw, "m_enterprise_attack_tactics"))
      if str(x).strip()]
  ...
  def _convert_exploit(self, raw: ExploitResultItem) -> Dict[str, Any]:
    ...
    if infra_seed:
      ...
      infra = {"type": "infrastructure", "spec_version": "2.1", "id": stix_id(
        "infrastructure",
        f"infra:{infra_seed}"), "created": created, "modified": modified, "name": str(
        first_nonempty(
          titled := title,
          safe_get(raw, "m_name"),
  ...
  def _convert_social(self, raw: SocialResultItem) -> Dict[str, Any]:
    ...
    url = first_nonempty(
      safe_get(raw, "m_message_sharable_link"),
      safe_get(raw, "m_channel_url"),
      safe_get(raw, "m_url"))
    ...
    content_src = first_nonempty(
      ...
      safe_get(raw, "m_important_content"),
  ...
  def _convert_chat(self, raw: ChatResultItem) -> Dict[str, Any]:
    ...
    if infra_seed:
      ...
      infra = {"type": "infrastructure", "spec_version": "2.1", "id": stix_id(
        "infrastructure",
        f"channel:{infra_seed}"), "created": created, "modified": modified, "name": str(
    ...
    report: Dict[str, Any] = {"type": "report", "spec_version": "2.1", "id": stix_id(
      ...
      channel_id) if channel_id else None, "x_orion_channel_name": str(safe_get(raw, "m_channel_name")) if safe_get(
      raw,
      "m_channel_name") else None, "x_orion_views": str(safe_get(raw, "m_views")) if safe_get(
      raw,
      "m_views") else None, "x_orion_sender_is_bot": bool(safe_get(raw, "m_sender_is_bot")) if safe_get(
      raw,
      "m_sender_is_bot") is not None else None, "x_orion_is_forwarded": bool(
      safe_get(raw, "m_is_forwarded")) if safe_get(
      raw,
      "m_is_forwarded") is not None else None, "x_orion_is_reply": bool(safe_get(raw, "m_is_reply")) if safe_get(
      raw,
      "m_is_reply") is not None else None, "x_orion_pinned": bool(safe_get(raw, "m_pinned")) if safe_get(
      raw,
      "m_pinned") is not None else None, }
    report = {k: v for k, v in report.items() if v is not None}
    add_obj(objects, seen, report, uniq=("report", report["id"]))

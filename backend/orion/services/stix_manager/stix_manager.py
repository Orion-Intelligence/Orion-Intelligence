...
class StixManager:
    ...
    def _normalize_summary(self, c: stix_helper, text: Any) -> str:
    ...
    def _collect_sco_lists(self, c: stix_helper, raw: Any, url_extra: Sequence[str] = (), domain_key: str = "m_domain", url_key: str = "m_url", ip_key: str = "m_ip", email_key: str = "m_email", asn_key: str = "m_asns", dir_key: str = "m_file_paths") -> Tuple[List[str], List[str], List[str], List[str], List[str], List[str]]:
        ...
        url_vals = [str(x).strip() for x in c.as_list(c.safe_get(raw, url_key)) if str(x).strip()]
        ip_vals = [str(x).strip() for x in c.as_list(c.safe_get(raw, ip_key)) if str(x).strip()]
        email_vals = [str(x).strip() for x in c.as_list(c.safe_get(raw, email_key)) if str(x).strip()]
        asn_vals = [str(x).strip().upper().lstrip("AS") for x in c.as_list(c.safe_get(raw, asn_key)) if str(x).strip()]
        dir_vals = [str(x).strip() for x in c.as_list(c.safe_get(raw, dir_key)) if str(x).strip()]
        for u in url_extra:
            su = str(u).strip()
            if su:
                url_vals.append(su)
    ...
    def _extend_urls_from_fields(self, c: stix_helper, url_vals: List[str], raw: Any, list_field_names: Sequence[str]) -> List[str]:
        for field in list_field_names:
            for x in c.as_list(c.safe_get(raw, field)):
                s = str(x).strip()
    ...
    def _report_bundle(self, c: stix_helper, report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _common_observed_and_indicators(self, c: stix_helper, raw: Any, doc_id: str, created: str, modified: str, tlp_amber_id: str, labels: List[str], summary: str, domain_vals: List[str], url_vals: List[str], ip_vals: List[str], email_vals: List[str], asn_vals: List[str], dir_vals: List[str], extra_scos: Optional[List[dict[str, Any]]] = None) -> Tuple[Optional[str], List[str]]:
    ...
    def _common_vulns_and_attack(self, c: stix_helper, raw: Any, created: str, modified: str, tlp_amber_id: str) -> Tuple[List[str], List[str]]:
    ...
    def _convert_defacement(self, raw: DefacementResultItem) -> Dict[str, Any]:
        ...
        labels = self._labels_from_content_types(c, raw, content_types, network, platform, ["orion:defacement"])
        ...
        domain_vals, url_vals, ip_vals, email_vals, asn_vals, file_paths = self._collect_sco_lists(c, raw, url_extra=[])
        ...
        return self._report_bundle(c, report)
    ...
    def _convert_exploit(self, raw: ExploitResultItem) -> Dict[str, Any]:
        ...
        content_types = self._content_types_from_raw(c, raw)
        labels = self._labels_from_content_types(c, raw, content_types, network, platform, ["orion:exploit"])
        lang = self._lang_single_or_none(c, raw)
        location_refs = c.add_locations(raw=raw, created=created, modified=modified, tlp_amber_id=tlp_amber_id, keys=["m_country", "m_location"])
        ...
        domain_vals, url_vals, ip_vals, email_vals, asn_vals, file_paths = self._collect_sco_lists(c, raw, url_extra=[])
        self._extend_urls_from_fields(c, url_vals, raw, ["m_encoded_urls", "m_weblink"])
        if url:
            url_vals.append(str(url))
        domain_vals = c.dedupe_keep(domain_vals)
        url_vals = c.dedupe_keep(url_vals)
    ...
    def _convert_leak(self, raw: LeakResultItem) -> Dict[str, Any]:
        ...
        tlp_amber_id, tlp_red_id = c.add_tlp(created)
        content_types = self._content_types_from_raw(c, raw)
        labels = self._labels_from_content_types(c, raw, content_types, network, platform, ["orion:leak"])
        lang = self._lang_single_or_none(c, raw)
        ...
        domain_vals, url_vals, ip_vals, email_vals, asn_vals, file_paths = self._collect_sco_lists(c, raw, url_extra=[])
        self._extend_urls_from_fields(c, url_vals, raw, ["m_encoded_urls", "m_dumplink", "m_websites"])
        ...
        if c.safe_get(raw, "m_screenshot"):
            extra_external.append({"source_name": "screenshot", "external_id": str(c.safe_get(raw, "m_screenshot"))})
    ...
    def _convert_social(self, raw: SocialResultItem) -> Dict[str, Any]:
        ...
        domain_vals, url_vals, ip_vals, email_vals, asn_vals, path_vals = self._collect_sco_lists(c, raw, url_extra=[])
    ...
    def _convert_general(self, raw: GeneralResultItem) -> Dict[str, Any]:
        ...
        summary = self._normalize_summary(c, summary_src)
        tlp_amber_id, tlp_red_id = c.add_tlp(created)
        content_types = self._content_types_from_raw(c, raw)
        labels_set: Set[str] = set()
        for ct in content_types:
        ...
        if team and str(team).strip():
            ...
            actor_ref = c.add_obj(actor, ("intrusion-set", f"team:{tname}"))
        domain_vals, url_vals, ip_vals, email_vals, asn_vals, path_vals = self._collect_sco_lists(c, raw, url_extra=[])
        self._extend_urls_from_fields(c, url_vals, raw, ["m_encoded_urls"])
        url_vals = c.dedupe_keep(url_vals)
        domain_vals = c.dedupe_keep(domain_vals)
        ip_vals = c.dedupe_keep(ip_vals)
        email_vals = c.dedupe_keep(email_vals)
        ...
        if actor_ref and infra_ref:
            rel = {"type": "relationship", "spec_version": "2.1", "id": c.stix_id("relationship", f"{actor_ref}|uses|{infra_ref}"), "created": created, "modified": modified, "relationship_type": "uses", "source_ref": actor_ref, "target_ref": infra_ref, "object_marking_refs": [tlp_amber_id]}
            c.add_obj(rel, ("relationship", f"{actor_ref}|uses|{infra_ref}"))
        observed_ref, indicator_refs = self._common_observed_and_indicators(c, raw, str(doc_id), created, modified, tlp_amber_id, labels, summary, domain_vals, url_vals, ip_vals, email_vals, asn_vals, path_vals)
        vuln_refs, attack_refs = self._common_vulns_and_attack(c, raw, created, modified, tlp_amber_id)
        note_ref = self._sensitive_note(c, raw, str(doc_id), created, modified, tlp_red_id)
        extra_external: List[dict[str, Any]] = []
        if c.safe_get(raw, "m_screenshot"):

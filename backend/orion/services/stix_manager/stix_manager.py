...
class StixManager:
    ...
    def _ts_pair(self, c: stix_helper, raw: Any, created_candidates: Sequence[str], modified_field: str) -> Tuple[str, str]:
        created = None
        for k in created_candidates:
            created = c.parse_ts(c.safe_get(raw, k))
            if created:
                break
        created = created or c.now_ts()
        modified = c.parse_ts(c.safe_get(raw, modified_field)) or created
        if modified < created:
            modified = created
        return created, modified

    def _clip_summary(self, c: stix_helper, content: Any) -> str:
        s = c.clean_text(str(content or ""))
        if len(s) > 4000:
            s = s[:4000] + "…"
        return s

    def _lang_single(self, c: stix_helper, raw: Any, key: str = "m_language") -> Optional[str]:
    ...
    def _dedup_keep(self, c: stix_helper, vals: Iterable[str]) -> List[str]:
        return c.dedupe_keep([v for v in vals if str(v).strip()])

    def _asns_digits(self, c: stix_helper, raw: Any, key: str = "m_asns") -> List[str]:
        asn_vals = [str(x).strip().upper().lstrip("AS") for x in c.as_list(c.safe_get(raw, key)) if str(x).strip()]
        return c.dedupe_keep([a for a in asn_vals if a.isdigit()])

    def _extend_urls_from_fields(self, c: stix_helper, raw: Any, url_vals: List[str], keys: Sequence[str]) -> None:
        for k in keys:
            for x in c.as_list(c.safe_get(raw, k)):
                sx = str(x).strip()
                if sx.startswith(("http://", "https://")):
                    url_vals.append(sx)
    ...
    def _infra(self, c: stix_helper, stix_name_seed: str, created: str, modified: str, name: str, description: Optional[str], infra_types: List[str], labels: List[str], tlp_amber_id: str, network: Any = None, platform: Any = None, extra: Optional[Dict[str, Any]] = None) -> str:
        infra: Dict[str, Any] = {
            "type": "infrastructure",
            "spec_version": "2.1",
            "id": c.stix_id("infrastructure", stix_name_seed),
            "created": created,
            "modified": modified,
            "name": name,
        ...
        return c.add_obj(infra, ("infrastructure", stix_name_seed))

    def _actor_intrusion_set(self, c: stix_helper, team_name: str, created: str, modified: str, description: Optional[str], tlp_amber_id: str) -> Optional[str]:
        ...
        actor: Dict[str, Any] = {
            "type": "intrusion-set",
            "spec_version": "2.1",
            "id": c.stix_id("intrusion-set", f"team:{t}"),
            "created": created,
            "modified": modified,
            "name": t,
            "description": description if description else None,
        ...
        return c.add_obj(actor, ("intrusion-set", f"team:{t}"))

    def _victim_identities(self, c: stix_helper, victims: List[str], created: str, modified: str, sector: Optional[str], tlp_amber_id: str) -> List[str]:
        out: List[str] = []
        for name in victims:
            n = str(name).strip()
            if not n:
            ...
            ident: Dict[str, Any] = {
                "type": "identity",
                "spec_version": "2.1",
                "id": c.stix_id("identity", f"victim:{n}"),
                "created": created,
                "modified": modified,
                "name": n,
            ...
            out.append(c.add_obj(ident, ("identity", f"victim:{n}")))
        return out

    def _link_located_at(self, c: stix_helper, created: str, modified: str, tlp_amber_id: str, sources: List[str], targets: List[str]) -> None:
        for sref in sources:
            for tref in targets:
                rel: Dict[str, Any] = {
                    "type": "relationship",
    ...
    def _link_uses(self, c: stix_helper, created: str, modified: str, tlp_amber_id: str, actor_ref: Optional[str], infra_ref: Optional[str]) -> None:
    ...
    def _note(self, c: stix_helper, note_id_seed: str, created: str, modified: str, abstract: str, content: Any, marking_refs: List[str]) -> str:
        note: Dict[str, Any] = {
            "type": "note",
            "spec_version": "2.1",
            "id": c.stix_id("note", note_id_seed),
            "created": created,
            "modified": modified,
            "abstract": abstract,
            "content": str(content),
    ...
    def _sensitive_note_only(self, c: stix_helper, raw: Any, doc_id: str, created: str, modified: str, tlp_red_id: str) -> Optional[str]:
        ...
        if not sensitive:
            return None
        return self._note(c, f"sensitive|{doc_id}|{created}", created, modified, "Sensitive artifacts (hashed)", sensitive, [tlp_red_id])

    def _vulns_and_attack(self, c: stix_helper, raw: Any, created: str, modified: str, tlp_amber_id: str, cves: Optional[List[str]] = None) -> Tuple[List[str], List[str]]:
        cves_in = cves if cves is not None else c.as_list(c.safe_get(raw, "m_cve"))
        vuln_refs = c.add_vulns(created=created, modified=modified, tlp_amber_id=tlp_amber_id, cves=cves_in)
    ...
    def _build_report_object_refs(
        self,
        c: stix_helper,
        fixed_refs: Sequence[Optional[str]],
        victim_refs: Optional[List[str]],
        location_refs: Optional[List[str]],
        indicator_refs: Optional[List[str]],
        vuln_refs: Optional[List[str]],
        attack_refs: Optional[List[str]],
        ...
        for r in fixed_refs:
            if r:
                out.append(r)
        if victim_refs:
            out.extend(victim_refs)
        if location_refs:
            out.extend(location_refs)
        if indicator_refs:
            out.extend(indicator_refs)
        if vuln_refs:
            out.extend(vuln_refs)
        if attack_refs:
            out.extend(attack_refs)
        if extra_refs:
            out.extend(extra_refs)
        return c.dedupe_keep(out)

    def _collect_sco_lists(self, c: stix_helper, raw: Any) -> Tuple[List[str], List[str], List[str], List[str], List[str], List[str]]:
        domain_vals = self._collect_str_list(c, raw, "m_domain")
        url_vals = self._collect_str_list(c, raw, "m_url")
        ip_vals = self._collect_str_list(c, raw, "m_ip")
        email_vals = self._collect_str_list(c, raw, "m_email")
        asn_vals = self._asns_digits(c, raw, "m_asns")
        path_vals = self._collect_str_list(c, raw, "m_file_paths")
        return domain_vals, url_vals, ip_vals, email_vals, asn_vals, path_vals

    def _append_url_and_dedupe_scos(
        self,
        c: stix_helper,
        url_vals: List[str],
        ...
        if primary_url:
            url_vals.append(str(primary_url))
    ...
    def _convert_defacement(self, raw: DefacementResultItem) -> Dict[str, Any]:
        c = stix_helper()
        created, modified = self._ts_pair(c, raw, ["m_leak_date", "m_creation_date", "m_update_date"], "m_update_date")
        title_any = c.first_nonempty(
        ...
        email_vals = self._collect_str_list(c, raw, "m_email")
        asn_vals = self._asns_digits(c, raw, "m_asns")
        file_paths = self._collect_str_list(c, raw, "m_file_paths")
        ...
        domain_vals, url_vals, ip_vals, email_vals, asn_vals, file_paths = self._append_url_and_dedupe_scos(
            c, url_vals, None, domain_vals, ip_vals, email_vals, asn_vals, file_paths
        )
        ...
        observed_ref, indicator_refs = self._observed_and_indicators(
            c, raw, str(doc_id), created, modified, tlp_amber_id, labels, summary, domain_vals, url_vals, ip_vals, email_vals, asn_vals, file_paths
        )
        ...
        external_refs = self._external_refs_common(c, raw, url, base_url, extra=None)
        report_object_refs = self._build_report_object_refs(
            c,
            [infra_ref, observed_ref],
            None,
            location_refs,
            indicator_refs,
            vuln_refs,
    ...
    def _convert_exploit(self, raw: ExploitResultItem) -> Dict[str, Any]:
        c = stix_helper()
        created, modified = self._ts_pair(c, raw, ["m_creation_date", "m_update_date", "m_leak_date"], "m_update_date")
        ...
        actor_ref = self._actor_intrusion_set(c, str(team).strip(), created, modified, summary if summary else None, tlp_amber_id) if team and str(team).strip() else None
        ...
        if infra_seed:
            ...
            infra_ref = self._infra(
                ...
                created,
                modified,
                str(c.first_nonempty(title, c.safe_get(raw, "m_name"), "Exploit infrastructure")),
                summary if summary else None,
                infra_types,
                labels,
                tlp_amber_id,
        ...
        note_ref = None
        if sensitive:
            note_ref = self._note(c, f"sensitive|{doc_id}|{created}", created, modified, "Sensitive artifacts (hashed)", sensitive, [tlp_red_id])
        external_refs = self._external_refs_common(c, raw, url, base_url, extra=None)
        report_object_refs = self._build_report_object_refs(
            c,
            [actor_ref, infra_ref, observed_ref, note_ref],
            None,
            location_refs,
            indicator_refs,
            vuln_refs,
    ...
    def _convert_leak(self, raw: LeakResultItem) -> Dict[str, Any]:
        c = stix_helper()
        created, modified = self._ts_pair(c, raw, ["m_creation_date", "m_update_date"], "m_update_date")
        ...
        location_refs = c.add_locations(raw=raw, created=created, modified=modified, tlp_amber_id=tlp_amber_id, keys=["m_country", "m_location"])
        victim_names = self._dedup_keep(c, self._collect_str_list(c, raw, "m_org") + self._collect_str_list(c, raw, "m_company_name"))
        ...
        if infra_seed:
            ...
            infra_ref = self._infra(
                ...
                created,
                modified,
                str(c.first_nonempty(c.safe_get(raw, "m_team"), title, "Leak infrastructure")),
                summary if summary else None,
                infra_types,
                labels,
                tlp_amber_id,
        ...
        domain_vals, url_vals, ip_vals, email_vals, asn_vals, file_paths = self._append_url_and_dedupe_scos(
            c, url_vals, str(url) if url else None, domain_vals, ip_vals, email_vals, asn_vals, file_paths
        )
        observed_ref, indicator_refs = self._observed_and_indicators(
            c, raw, str(doc_id), created, modified, tlp_amber_id, labels, summary, domain_vals, url_vals, ip_vals, email_vals, asn_vals, file_paths
        )
    ...
    def _convert_social(self, raw: SocialResultItem) -> Dict[str, Any]:
        c = stix_helper()
        created, modified = self._ts_pair(c, raw, ["m_creation_date", "m_update_date", "m_message_date"], "m_update_date")
        ...
        if author:
            ...
            if author_name:
                ident: Dict[str, Any] = {
                    "type": "identity",
                    "spec_version": "2.1",
                    "id": c.stix_id("identity", f"author:{author_name}"),
                    "created": created,
                    "modified": modified,
                    "name": author_name,
        ...
        encoded_urls = self._collect_str_list(c, raw, "m_encoded_urls")
        xmpp_vals = self._collect_str_list(c, raw, "m_xmpp_addresses")
        crypto_vals = self._collect_str_list(c, raw, "m_crypto_address")
        user_agents = self._collect_str_list(c, raw, "m_user_agents")
        ...
        asn_vals = c.dedupe_keep([a for a in asn_vals if a.isdigit()])
        path_vals = c.dedupe_keep(path_vals)
        xmpp_vals = c.dedupe_keep(xmpp_vals)
        crypto_vals = c.dedupe_keep(crypto_vals)
        user_agents = c.dedupe_keep(user_agents)
        hashtags = c.dedupe_keep(hashtags)
        mentions = c.dedupe_keep(mentions)
        infra_seed = c.first_nonempty(base_url, url, (domain_vals[0] if domain_vals else None))
        infra_ref = None
        if infra_seed:
            infra_types = ["unknown"]
            ...
            infra_ref = self._infra(
                ...
                created,
                modified,
                str(c.first_nonempty(platform, title, "Social infrastructure")),
                summary if summary else None,
                infra_types,
                labels,
                tlp_amber_id,
        ...
        observed_ref, indicator_refs = self._observed_and_indicators(
            c, raw, str(doc_id), created, modified, tlp_amber_id, labels, summary, domain_vals, url_vals, ip_vals, email_vals, asn_vals, path_vals, extra_scos=extra_scos
        )
        vuln_refs, attack_refs = self._vulns_and_attack(c, raw, created, modified, tlp_amber_id)
        sensitive = self._sensitive(c, raw)
        note_ref = None
        if sensitive or hashtags or mentions:
            ...
            note_ref = self._note(
                c,
                f"social-meta|{doc_id}|{created}",
                created,
                modified,
                "Social metadata (and sensitive hashed)",
                content_note,
                [tlp_red_id] if sensitive else [tlp_amber_id],
            )
    ...
    def _convert_general(self, raw: GeneralResultItem) -> Dict[str, Any]:
        ...
        if infra_seed:
            ...
            infra_ref = self._infra(
                ...
                created,
                modified,
                str(c.first_nonempty(c.safe_get(raw, "m_team"), title, "Observed infrastructure")),
                summary if summary else None,
                infra_types,
                labels,
                tlp_amber_id,
        ...
        self._link_uses(c, created, modified, tlp_amber_id, actor_ref, infra_ref)
        observed_ref, indicator_refs = self._observed_and_indicators(
            c, raw, str(doc_id), created, modified, tlp_amber_id, labels, summary, domain_vals, url_vals, ip_vals, email_vals, asn_vals, path_vals
        )
        vuln_refs, attack_refs = self._vulns_and_attack(c, raw, created, modified, tlp_amber_id)
        note_ref = self._sensitive_note_only(c, raw, str(doc_id), created, modified, tlp_red_id)
        extra_external: List[Dict[str, Any]] = [{"source_name": "screenshot", "external_id": str(c.safe_get(raw, "m_screenshot"))}] if c.safe_get(raw, "m_screenshot") else []
    ...
    def _convert_chat(self, raw: ChatResultItem) -> Dict[str, Any]:
        ...
        tlp_amber_id, tlp_red_id = c.add_tlp(created)
        content_types = self._content_types(c, raw)
        labels = self._labels(c, ["orion:chat"], content_types, network, platform)
        lang = self._lang_single(c, raw)
        ...
        if sender:
            ...
            if sender_name:
                ident: Dict[str, Any] = {
                    "type": "identity",
                    "spec_version": "2.1",
                    "id": c.stix_id("identity", f"sender:{sender_name}"),
                    "created": created,
                    "modified": modified,
                    "name": sender_name,
                    "identity_class": "individual",
        ...
        if infra_seed:
            ...
            infra_ref = self._infra(
                ...
                tlp_amber_id,
                network=network,
                extra={"x_orion_channel_id": str(channel_id) if channel_id else None},
            )
        domain_vals = self._collect_str_list(c, raw, "m_domain")
        url_vals = self._collect_str_list(c, raw, "m_url")
        ip_vals = self._collect_str_list(c, raw, "m_ip")
        ...
        file_paths = self._collect_str_list(c, raw, "m_file_paths")
        self._extend_urls_from_fields(c, raw, url_vals, ["m_encoded_urls", "m_weblink"])
        mentions = self._collect_str_list(c, raw, "m_mention")
        hashtags = [str(x).strip().lstrip("#") for x in c.as_list(c.safe_get(raw, "m_hashtag")) if str(x).strip()]
        user_agents = self._collect_str_list(c, raw, "m_user_agents")
        ...
        for ua in user_agents:
            extra_scos.append({"type": "user-agent", "id": c.sco_id("user-agent", ua), "string": ua})
        observed_ref, indicator_refs = self._observed_and_indicators(
            c, raw, str(doc_id), created, modified, tlp_amber_id, labels, summary, domain_vals, url_vals, ip_vals, email_vals, asn_vals, file_paths, extra_scos=extra_scos
        )
        vuln_only = [x for x in cves if x.startswith("CVE-")]
        vuln_refs, attack_refs = self._vulns_and_attack(c, raw, created, modified, tlp_amber_id, cves=c.dedupe_keep(vuln_only))
        sensitive = self._sensitive(c, raw)
        note_ref = None
        if sensitive or hashtags or mentions:
            content_note: Dict[str, Any] = {}
            ...
            note_ref = self._note(
                c,
                f"chat-meta|{doc_id}|{created}",
                created,
                modified,
                "Chat metadata (and sensitive hashed)",
                content_note,
                [tlp_red_id] if sensitive else [tlp_amber_id],
            )
        extra_external = [{"source_name": "message_id", "external_id": str(c.safe_get(raw, "m_message_id"))}] if c.safe_get(raw, "m_message_id") else []
        external_refs = self._external_refs_common(c, raw, url, channel_url, base_url_name="channel_url", extra=extra_external)
        report_object_refs = self._build_report_object_refs(
            c,
            [infra_ref, observed_ref, note_ref, created_by_ref],
            None,

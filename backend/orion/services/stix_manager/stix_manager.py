...
class StixManager:
    ...
    def _convert_defacement(self, raw: DefacementResultItem) -> Dict[str, Any]:
        ...
        if not content_types: content_types.add("defacement")
        ...
        source_urls = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_source_url")) if str(x).strip()]
        for eu in encoded_urls:
            if eu.startswith(("http://", "https://")): url_vals.append(eu)
        for ml in mirror_links:
            if ml.startswith(("http://", "https://")): url_vals.append(ml)
        for su in source_urls:
            if su.startswith(("http://", "https://")): url_vals.append(su)
        ...
        infra_ref = None
        if infra_seed:
            infra = {"type": "infrastructure", "spec_version": "2.1", "id": c.stix_id("infrastructure", f"infra:{infra_seed}"), "created": created, "modified": modified, "name": title, "description": summary if summary else None, "infrastructure_types": ["unknown"], "first_seen": created, "last_seen": modified, "labels": labels, "object_marking_refs": [tlp_amber_id], "x_orion_network": str(network) if network else None}
            infra = {k: v for k, v in infra.items() if v is not None}
            infra_ref = c.add_obj(infra, ("infrastructure", f"infra:{infra_seed}"))
        sco_refs = c.add_scos(tlp_amber_id=tlp_amber_id, url_vals=url_vals, domain_vals=domain_vals, ip_vals=ip_vals, email_vals=email_vals, asn_vals=asn_vals, dir_vals=file_paths)
        ...
        if c.safe_get(raw, "m_hash"): external_refs.append({"source_name": "content-hash", "external_id": str(c.safe_get(raw, "m_hash"))})
        if c.safe_get(raw, "m_scrap_file"): external_refs.append({"source_name": "scraper", "external_id": str(c.safe_get(raw, "m_scrap_file"))})
        report_object_refs: list[str] = []
        if infra_ref: report_object_refs.append(infra_ref)
        if observed_ref: report_object_refs.append(observed_ref)
        report_object_refs.extend(location_refs); report_object_refs.extend(indicator_refs); report_object_refs.extend(vuln_refs); report_object_refs.extend(attack_refs)
    ...
    def _convert_exploit(self, raw: ExploitResultItem) -> Dict[str, Any]:
        ...
        if team and str(team).strip():
            ...
            actor = {"type": "intrusion-set", "spec_version": "2.1", "id": c.stix_id("intrusion-set", f"team:{tname}"), "created": created, "modified": modified, "name": tname, "description": summary if summary else None, "object_marking_refs": [tlp_amber_id]}
        ...
        file_paths = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_file_paths")) if str(x).strip()]
        encoded_urls = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_encoded_urls")) if str(x).strip()]
        weblinks = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_weblink")) if str(x).strip()]
        for eu in encoded_urls:
            if eu.startswith(("http://", "https://")): url_vals.append(eu)
        for wl in weblinks:
            if wl.startswith(("http://", "https://")): url_vals.append(wl)
        if url: url_vals.append(str(url))
        domain_vals = c.dedupe_keep(domain_vals)
        url_vals = c.dedupe_keep(url_vals)
        ip_vals = c.dedupe_keep(ip_vals)
        ...
        if infra_seed:
            infra_types = ["unknown"]
            if "c2" in content_types: infra_types = ["command-and-control"]
            elif str(network).lower() == "onion": infra_types = ["anonymization"]
            infra = {"type": "infrastructure", "spec_version": "2.1", "id": c.stix_id("infrastructure", f"infra:{infra_seed}"), "created": created, "modified": modified, "name": str(c.first_nonempty(title, c.safe_get(raw, "m_name"), "Exploit infrastructure")), "description": summary if summary else None, "infrastructure_types": infra_types, "first_seen": created, "last_seen": modified, "labels": labels, "object_marking_refs": [tlp_amber_id], "x_orion_network": str(network) if network else None}
            infra = {k: v for k, v in infra.items() if v is not None}
            infra_ref = c.add_obj(infra, ("infrastructure", f"infra:{infra_seed}"))
        sco_refs = c.add_scos(tlp_amber_id=tlp_amber_id, url_vals=url_vals, domain_vals=domain_vals, ip_vals=ip_vals, email_vals=email_vals, asn_vals=asn_vals, dir_vals=file_paths)
        observed_ref = c.add_observed(doc_id=str(doc_id), created=created, modified=modified, tlp_amber_id=tlp_amber_id, sco_refs=sco_refs)
        ...
        for r in [actor_ref, infra_ref, observed_ref, note_ref]:
            if r: report_object_refs.append(r)
        report_object_refs.extend(location_refs); report_object_refs.extend(indicator_refs); report_object_refs.extend(vuln_refs); report_object_refs.extend(attack_refs)
        report_object_refs = c.dedupe_keep(report_object_refs)
    ...
    def _convert_leak(self, raw: LeakResultItem) -> Dict[str, Any]:
        ...
        if infra_seed:
            ...
            infra = {"type": "infrastructure", "spec_version": "2.1", "id": c.stix_id("infrastructure", f"infra:{infra_seed}"), "created": created, "modified": modified, "name": str(c.first_nonempty(c.safe_get(raw, "m_team"), title, "Leak infrastructure")), "description": summary if summary else None, "infrastructure_types": infra_types, "first_seen": created, "last_seen": modified, "labels": labels, "object_marking_refs": [tlp_amber_id], "x_orion_network": str(network) if network else None}
            infra = {k: v for k, v in infra.items() if v is not None}
            infra_ref = c.add_obj(infra, ("infrastructure", f"infra:{infra_seed}"))
        if actor_ref and infra_ref:
            rel = {"type": "relationship", "spec_version": "2.1", "id": c.stix_id("relationship", f"{actor_ref}|uses|{infra_ref}"), "created": created, "modified": modified, "relationship_type": "uses", "source_ref": actor_ref, "target_ref": infra_ref, "object_marking_refs": [tlp_amber_id]}
        ...
        websites = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_websites")) if str(x).strip()]
        for eu in encoded_urls:
            if eu.startswith(("http://", "https://")): url_vals.append(eu)
        for dl in dump_links:
            if dl.startswith(("http://", "https://")): url_vals.append(dl)
        for ws in websites:
            if ws.startswith(("http://", "https://")): url_vals.append(ws)
        domain_vals = c.dedupe_keep(domain_vals)
        url_vals = c.dedupe_keep(url_vals)
        ip_vals = c.dedupe_keep(ip_vals)
        email_vals = c.dedupe_keep(email_vals)
        ...
        if c.safe_get(raw, "m_hash"): external_refs.append({"source_name": "content-hash", "external_id": str(c.safe_get(raw, "m_hash"))})
        if c.safe_get(raw, "m_scrap_file"): external_refs.append({"source_name": "scraper", "external_id": str(c.safe_get(raw, "m_scrap_file"))})
        if c.safe_get(raw, "m_screenshot"): external_refs.append({"source_name": "screenshot", "external_id": str(c.safe_get(raw, "m_screenshot"))})
        report_object_refs: list[str] = []
        for r in [actor_ref, infra_ref, observed_ref, note_ref]:
            if r: report_object_refs.append(r)
        report_object_refs.extend(victim_refs); report_object_refs.extend(location_refs); report_object_refs.extend(indicator_refs); report_object_refs.extend(vuln_refs); report_object_refs.extend(attack_refs)
        report_object_refs = c.dedupe_keep(report_object_refs)
    ...
    def _convert_social(self, raw: SocialResultItem) -> Dict[str, Any]:
        ...
        email_vals = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_email")) if str(x).strip()]
        asn_vals = [str(x).strip().upper().lstrip("AS") for x in c.as_list(c.safe_get(raw, "m_asns")) if str(x).strip()]
        path_vals = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_file_paths")) if str(x).strip()]
        social_profiles = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_social_media_profiles")) if str(x).strip()]
        encoded_urls = [str(x).strip() for x in c.as_list(c.safe_get(raw, "m_encoded_urls")) if str(x).strip()]
        ...
        for sp in social_profiles:
            if sp.startswith(("http://", "https://")): url_vals.append(sp)
        domain_vals = c.dedupe_keep(domain_vals)
        url_vals = c.dedupe_keep(url_vals)
        ip_vals = c.dedupe_keep(ip_vals)
        email_vals = c.dedupe_keep(email_vals)
        ...
        if infra_seed:
            infra_types = ["unknown"]
            if str(network).lower() == "onion": infra_types = ["anonymization"]
            infra = {"type": "infrastructure", "spec_version": "2.1", "id": c.stix_id("infrastructure", f"infra:{infra_seed}"), "created": created, "modified": modified, "name": str(c.first_nonempty(platform, title, "Social infrastructure")), "description": summary if summary else None, "infrastructure_types": infra_types, "first_seen": created, "last_seen": modified, "labels": labels, "object_marking_refs": [tlp_amber_id], "x_orion_network": str(network) if network else None, "x_orion_platform": str(platform) if platform else None}
            infra = {k: v for k, v in infra.items() if v is not None}
            infra_ref = c.add_obj(infra, ("infrastructure", f"infra:{infra_seed}"))
        extra_scos: list[dict[str, Any]] = []
        for x in xmpp_vals: extra_scos.append({"type": "x-mpp-addr", "id": c.sco_id("x-mpp-addr", x), "value": x})
        for w in crypto_vals: extra_scos.append({"type": "cryptocurrency-wallet", "id": c.sco_id("cryptocurrency-wallet", w), "address": w})
        for ua in user_agents: extra_scos.append({"type": "user-agent", "id": c.sco_id("user-agent", ua), "string": ua})
        sco_refs = c.add_scos(tlp_amber_id=tlp_amber_id, url_vals=url_vals, domain_vals=domain_vals, ip_vals=ip_vals, email_vals=email_vals, asn_vals=asn_vals, dir_vals=path_vals, extra_scos=extra_scos)
        observed_ref = c.add_observed(doc_id=str(doc_id), created=created, modified=modified, tlp_amber_id=tlp_amber_id, sco_refs=sco_refs)
        indicator_refs = c.add_indicators(created=created, modified=modified, tlp_amber_id=tlp_amber_id, labels=labels, summary=summary, domain_vals=domain_vals, url_vals=url_vals, ip_vals=ip_vals, email_vals=email_vals, indicator_types_default="malicious-activity")
        indicator_refs.extend(c.add_yara_indicators(created=created, modified=modified, tlp_amber_id=tlp_amber_id, labels=labels, yara_rules=c.as_list(c.safe_get(raw, "m_yara_rule"))))
        vuln_refs = c.add_vulns(created=created, modified=modified, tlp_amber_id=tlp_amber_id, cves=c.as_list(c.safe_get(raw, "m_cve")))
        ...
        external_refs: list[dict[str, Any]] = []
        if url: external_refs.append({"source_name": "source", "url": str(url)})
        if base_url and base_url != url: external_refs.append({"source_name": "channel_url", "url": str(base_url)})
        if c.safe_get(raw, "m_hash"): external_refs.append({"source_name": "content-hash", "external_id": str(c.safe_get(raw, "m_hash"))})
        if c.safe_get(raw, "m_scrap_file"): external_refs.append({"source_name": "scraper", "external_id": str(c.safe_get(raw, "m_scrap_file"))})
        if c.safe_get(raw, "m_message_sharable_link"): external_refs.append({"source_name": "share_link", "url": str(c.safe_get(raw, "m_message_sharable_link"))})
        report_object_refs: list[str] = []
    ...
    def _convert_general(self, raw: GeneralResultItem) -> Dict[str, Any]:
        ...
        if infra_seed:
            infra_types = ["unknown"]
            if str(network).lower() == "onion": infra_types = ["anonymization"]
            elif "darkweb" in content_types: infra_types = ["hosting-malware"]
            infra = {"type": "infrastructure", "spec_version": "2.1", "id": c.stix_id("infrastructure", f"infra:{infra_seed}"), "created": created, "modified": modified, "name": str(c.first_nonempty(c.safe_get(raw, "m_team"), title, "Observed infrastructure")), "description": summary if summary else None, "infrastructure_types": infra_types, "first_seen": created, "last_seen": modified, "labels": labels, "object_marking_refs": [tlp_amber_id], "x_orion_network": str(network) if network else None}
            infra = {k: v for k, v in infra.items() if v is not None}
            infra_ref = c.add_obj(infra, ("infrastructure", f"infra:{infra_seed}"))
        if actor_ref and infra_ref:
            rel = {"type": "relationship", "spec_version": "2.1", "id": c.stix_id("relationship", f"{actor_ref}|uses|{infra_ref}"), "created": created, "modified": modified, "relationship_type": "uses", "source_ref": actor_ref, "target_ref": infra_ref, "object_marking_refs": [tlp_amber_id]}
            c.add_obj(rel, ("relationship", f"{actor_ref}|uses|{infra_ref}"))
        sco_refs = c.add_scos(tlp_amber_id=tlp_amber_id, url_vals=url_vals, domain_vals=domain_vals, ip_vals=ip_vals, email_vals=email_vals, asn_vals=asn_vals, dir_vals=path_vals)
        observed_ref = c.add_observed(doc_id=str(doc_id), created=created, modified=modified, tlp_amber_id=tlp_amber_id, sco_refs=sco_refs)
        indicator_refs = c.add_indicators(created=created, modified=modified, tlp_amber_id=tlp_amber_id, labels=labels, summary=summary, domain_vals=domain_vals, url_vals=url_vals, ip_vals=ip_vals, email_vals=email_vals, indicator_types_default="malicious-activity")
        indicator_refs.extend(c.add_yara_indicators(created=created, modified=modified, tlp_amber_id=tlp_amber_id, labels=labels, yara_rules=c.as_list(c.safe_get(raw, "m_yara_rule"))))
        vuln_refs = c.add_vulns(created=created, modified=modified, tlp_amber_id=tlp_amber_id, cves=c.as_list(c.safe_get(raw, "m_cve")))
    ...
    def _convert_chat(self, raw: ChatResultItem) -> Dict[str, Any]:
        ...
        if infra_seed:
            infra_types = ["unknown"]
            if str(platform).strip().lower() in {"telegram", "t.me"} or (channel_url and "t.me" in str(channel_url)): infra_types = ["communications"]
            infra = {"type": "infrastructure", "spec_version": "2.1", "id": c.stix_id("infrastructure", f"channel:{infra_seed}"), "created": created, "modified": modified, "name": str(channel_name), "description": summary if summary else None, "infrastructure_types": infra_types, "first_seen": created, "last_seen": modified, "labels": labels, "object_marking_refs": [tlp_amber_id], "x_orion_network": str(network) if network else None, "x_orion_channel_id": str(channel_id) if channel_id else None}
            infra = {k: v for k, v in infra.items() if v is not None}
        ...
        for wl in weblinks:
            if wl.startswith(("http://", "https://")): url_vals.append(wl)
        if url: url_vals.append(str(url))
        if channel_url: url_vals.append(str(channel_url))
        domain_vals = c.dedupe_keep(domain_vals)
        url_vals = c.dedupe_keep(url_vals)
        ip_vals = c.dedupe_keep(ip_vals)
        email_vals = c.dedupe_keep(email_vals)
        ...
        vuln_only = []
        for x in cves:
            if x.startswith("CVE-"): vuln_only.append(x)
        ...
        external_refs: list[dict[str, Any]] = []
        if url: external_refs.append({"source_name": "source", "url": str(url)})
        if channel_url and channel_url != url: external_refs.append({"source_name": "channel_url", "url": str(channel_url)})
        if c.safe_get(raw, "m_hash"): external_refs.append({"source_name": "content-hash", "external_id": str(c.safe_get(raw, "m_hash"))})
        if c.safe_get(raw, "m_scrap_file"): external_refs.append({"source_name": "scraper", "external_id": str(c.safe_get(raw, "m_scrap_file"))})
        if c.safe_get(raw, "m_message_id"): external_refs.append({"source_name": "message_id", "external_id": str(c.safe_get(raw, "m_message_id"))})
        report_object_refs: list[str] = []
        for r in [infra_ref, observed_ref, note_ref, created_by_ref]:
            if r: report_object_refs.append(r)
        report_object_refs.extend(vuln_refs); report_object_refs.extend(indicator_refs); report_object_refs.extend(attack_refs)
        report_object_refs = c.dedupe_keep(report_object_refs)

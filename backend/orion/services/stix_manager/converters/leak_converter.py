...
class _LeakConverter:
    def convert(self, raw):


        h = StixHelper()
        created = (h.parse_ts(h.safe_get(raw, "m_creation_date")) or h.parse_ts(h.safe_get(raw, "m_update_date")) or h.now_ts())
        ...
        title = str(
            h.first_nonempty(
                h.safe_get(raw, "m_title"), h.safe_get(raw, "m_url"), h.safe_get(raw, "m_base_url"), "Leak - unknown title"))
        url = h.first_nonempty(h.safe_get(raw, "m_url"), h.safe_get(raw, "m_base_url"))
        base_url = h.safe_get(raw, "m_base_url")
        ...
        content_src = h.first_nonempty(h.safe_get(raw, "m_important_content"), h.safe_get(raw, "m_content"), "")
        ...
        for org in (h.as_list(h.safe_get(raw, "m_org")) + h.as_list(h.safe_get(raw, "m_company_name"))):
            name = str(org).strip()
            if not name:
                continue
            ident: Dict[str, Any] = {"type": "identity", "spec_version": "2.1", "id": h.stix_id(
                "identity",
            ...
            victim_refs.append(h.add_obj(objects, seen, ident, uniq=("identity", f"victim:{name}")))

        for vref in victim_refs:
            for lref in location_refs:
                rel = {"type": "relationship", "spec_version": "2.1", "id": h.stix_id(
                    "relationship",
                    ...
                    tlp_amber_id], }
                h.add_obj(objects, seen, rel, uniq=("relationship", f"{vref}|located-at|{lref}"))

        team = h.first_nonempty(h.safe_get(raw, "m_team"), h.safe_get(raw, "m_author"))
        actor_ref = None
        if team:
            tname = str(team).strip()
            if tname:
                actor = {"type": "intrusion-set", "spec_version": "2.1", "id": h.stix_id(
                    "intrusion-set",
                ...
                actor_ref = h.add_obj(objects, seen, actor, uniq=("intrusion-set", f"team:{tname}"))

        infra_seed = h.first_nonempty(
            base_url, url, (h.as_list(h.safe_get(raw, "m_domain"))[0] if h.as_list(h.safe_get(raw, "m_domain")) else None))
        infra_ref = None
        if infra_seed:
            infra_types = ["unknown"]
        ...
        if actor_ref and infra_ref:
            rel = {"type": "relationship", "spec_version": "2.1", "id": h.stix_id(
                ...
                tlp_amber_id], }
            h.add_obj(objects, seen, rel, uniq=("relationship", f"{actor_ref}|uses|{infra_ref}"))
        ...
        dump_links = [str(x).strip() for x in h.as_list(h.safe_get(raw, "m_dumplink")) if str(x).strip()]
        websites = [str(x).strip() for x in h.as_list(h.safe_get(raw, "m_websites")) if str(x).strip()]
        ...
        if h.safe_get(raw, "m_screenshot"):
            external_refs.append({"source_name": "screenshot", "external_id": str(h.safe_get(raw, "m_screenshot"))})

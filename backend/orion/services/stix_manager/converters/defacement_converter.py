...
class _DefacementConverter:
    def convert(self, raw):

        h = StixHelper()
        created = (h.parse_ts(h.safe_get(raw, "m_leak_date")) or h.parse_ts(h.safe_get(raw, "m_creation_date")) or h.parse_ts(
            h.safe_get(raw, "m_update_date")) or h.now_ts())
        modified = h.parse_ts(h.safe_get(raw, "m_update_date")) or created
        if modified < created:
            modified = created

        title = h.first_nonempty(
            h.safe_get(raw, "m_title"),
            h.safe_get(raw, "m_url"),
            h.safe_get(raw, "m_base_url"),
            (h.as_list(h.safe_get(raw, "m_mirror_links"))[0] if h.as_list(h.safe_get(raw, "m_mirror_links")) else None),
            (str(h.safe_get(raw, "m_content")).splitlines()[0] if h.safe_get(raw, "m_content") else None),
        ...
        url = h.first_nonempty(
            h.safe_get(raw, "m_url"),
            h.safe_get(raw, "m_base_url"),
            (h.as_list(h.safe_get(raw, "m_source_url"))[0] if h.as_list(h.safe_get(raw, "m_source_url")) else None),
            (h.as_list(h.safe_get(raw, "m_mirror_links"))[0] if h.as_list(h.safe_get(raw, "m_mirror_links")) else None), )
        ...
        content_src = h.first_nonempty(h.safe_get(raw, "m_content"), h.safe_get(raw, "m_important_content"), "")
        ...
        mirror_links = [str(x).strip() for x in h.as_list(h.safe_get(raw, "m_mirror_links")) if str(x).strip()]
        source_urls = [str(x).strip() for x in h.as_list(h.safe_get(raw, "m_source_url")) if str(x).strip()]
        ...
        attack_vector = h.first_nonempty(
            (h.as_list(h.safe_get(raw, "m_ioc_type"))[0] if h.as_list(h.safe_get(raw, "m_ioc_type")) else None),
            (h.as_list(h.safe_get(raw, "m_web_server"))[0] if h.as_list(h.safe_get(raw, "m_web_server")) else None),
            "Unknown", )

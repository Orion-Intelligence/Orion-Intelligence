...
class _GeneralConverter:
    def convert(self, raw):


        h = StixHelper(allow_date_only=False)
        ...
        summary_src = h.first_nonempty(
            h.safe_get(raw, "m_important_content"), h.safe_get(raw, "m_meta_description"), h.safe_get(raw, "m_content"), "")
        summary = h.clean_text(str(summary_src or ""))
        if len(summary) > 4000:
        ...
        for p in h.as_list(h.safe_get(raw, "m_platform")):
            sp = str(p).strip().lower()
            if sp:
                labels.add(f"platform:{sp}")
        for h in h.as_list(h.safe_get(raw, "m_hashtag")):
        ...
        infra_seed = h.first_nonempty(url, base_url, domain_vals[0] if domain_vals else None)
        infra_ref = None
        if infra_seed:
            infra_types = ["unknown"]

...
class crawl_model:
    ...
    try:
      async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
          return JSONResponse(
            status_code=response.status_code, content={"detail": "Something happened while calling urlscan/domain"})
        return response.json()
    except Exception:
      return JSONResponse(
  ...
  async def invoke_social_index(self, social_index: social_data_model):
    ...
    return await self._update_or_create_model(
      ...
      new_index_type=[
        helper_controller.get_base_url(social_index.seed_url).replace("https://", "").replace(".com", "").rstrip('/')],

...
class ScanJobManager:
    ...
    @classmethod
    def _complete_api_reference(self, api_reference: str) -> str:
        return f"/api/{self._normalize_api_reference(api_reference)}"
    ...
    async def create_job(self, current_user, api_reference: str, payload: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        job = db_scan_job_model(
            ...
            api_reference=self._complete_api_reference(api_reference),
    ...
    async def list_incomplete_scans(self, current_user, limit: int = 1) -> Dict[str, Any]:
        ...
        records = await self._engine.find(db_scan_job_model, db_scan_job_model.user_uuid == str(current_user.id), sort=desc(db_scan_job_model.created_at), limit=500)
        incomplete_records = []
        for record in records:
    ...
    async def count_jobs(self, current_user) -> Dict[str, int]:
        ...
        records = await self._engine.find(db_scan_job_model, query, sort=desc(db_scan_job_model.created_at), limit=1000)

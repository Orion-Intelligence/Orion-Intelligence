...
async def create_default_tenant(engine):
    ...
    data.iocs = [IocCategory(
        ...
        values=[enc.encrypt(v.encode()).decode() for v in (ioc.values or [])], ) for ioc
        in (data.iocs or [])]
...
async def create_default_users(engine,
                               tenant_id):
    existing_admin = await engine.find_one(db_user_account,
...
async def tenant_boostrap(engine):
    ...
    try:
        ...
        await create_default_users(engine,
                                   data.id)
        return data
    except Exception:
        if data is not None:
            await engine.remove(db_user_account,
                                db_user_account.tenant_uuid == str(data.id))
            await engine.remove(db_keys,
                                db_keys.id == str(data.id))

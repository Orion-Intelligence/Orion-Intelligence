    ...
    async def update_trending_insights(self, args):
        try:
            ...
            await redis_controller.getInstance().invoke_trigger(
                ...
                [REDIS_KEYS.INSIGHT_STAT, insight_comparison.model_dump_json(), None]
            )
            if args == REDIS_KEYS.INSIGHT_OLD_DAY:
                await redis_controller.getInstance().invoke_trigger(
                    REDIS_COMMANDS.S_SET_STRING,
                    [REDIS_KEYS.INSIGHT_OLD_DAY, insight_new.model_dump_json(), None]
                )

            if args == REDIS_KEYS.INSIGHT_OLD_WEEK:
                await redis_controller.getInstance().invoke_trigger(
                    REDIS_COMMANDS.S_SET_STRING,
                    [REDIS_KEYS.INSIGHT_OLD_WEEK, insight_new.model_dump_json(), None]
                )

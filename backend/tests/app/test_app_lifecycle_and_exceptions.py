...
def test_validation_exception_handler_debug_json(monkeypatch):
    ...
    app = FastAPI()
    app.add_exception_handler(RequestValidationError, eh.validation_exception_handler)
...
def test_global_exception_handler_non_debug_redirect(monkeypatch):
    ...
    client = TestClient(app)
    resp = client.get("/boom", follow_redirects=False)

...
from tests.model.fakes import FakeAsyncClient, FakeBloom, FakeDoc, FakeElastic, FakeMongoEngine, FakeResponse
from tests.scripts.crawl.fakes import _FakeRoutingEngine
from tests.scripts.crawl.helpers import _json_request, _request, _run
...
def test_fetch_cti_label_and_proxy_swarm_index_cover_forwarding_and_dedup(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda url, **kwargs: posted.update({"url": url, "json": kwargs.get("json")}) or _FakeReqResponse(),
    )
    ...
    monkeypatch.setattr(crawl_manager, "_" + crawl_manager.__name__ + "__swarm_bloom", bloom)

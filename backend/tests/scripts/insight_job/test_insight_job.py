...
from tests.model.fakes import FakeRedis
...
async def test_get_insight_reads_doc_count_aggregations(monkeypatch):
    class _FakeElastic:
        @staticmethod
        async def search_query(document, data_filter):
    ...
    monkeypatch.setattr(
        ...
        staticmethod(lambda: _FakeElastic()),
    )
...
async def test_update_trending_insights_reads_old_values_and_writes_day_snapshot(monkeypatch):
    ...
    async def _fake_get_insight():
        return True, new_insight

    monkeypatch.setattr(insight_job, "get_insight", staticmethod(_fake_get_insight))
    monkeypatch.setattr(

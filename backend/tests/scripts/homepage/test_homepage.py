...
from tests.model.fakes import FakeElastic, FakeRedis
from tests.scripts.homepage.helpers import _run
...
def test_invoke_analytics_reads_valid_comparison_from_redis(monkeypatch):
    ...
    monkeypatch.setattr(
        "orion.api.interactive.hompage_manager.homepage_manager.redis_controller.getInstance",
        staticmethod(lambda: fake_redis),
    )

    result = _run(homepage_manager.invoke_analytics())
...
def test_invoke_analytics_returns_none_for_missing_or_invalid_json(monkeypatch):
    ...
    monkeypatch.setattr(
        "orion.api.interactive.hompage_manager.homepage_manager.log.g",
        staticmethod(lambda: type("Logger", (), {"ex": staticmethod(lambda *_args, **_kwargs: None)})()),
    )

    assert _run(homepage_manager.invoke_analytics()) is None
...
def test_insight_consolidated_result_uses_cache_before_elastic(monkeypatch):
    ...
    result = _run(homepage_manager.insight_consolidated_result())
...
def test_insight_consolidated_result_builds_and_caches_display_data(monkeypatch):
    ...
    monkeypatch.setattr(
        "orion.api.interactive.hompage_manager.homepage_manager.insight_generator.on_insight_consolidated_data",
        lambda self: (["leak_model", "chat_model", "exploit_model", "generic_model", "defacement_model"], [{"q": 1}] * 5),
    )
    monkeypatch.setattr(
        "orion.api.interactive.hompage_manager.homepage_manager.elastic_controller.get_instance",
...
def test_country_specific_insights_and_pagination_resolve_hashes(monkeypatch):
    ...
    monkeypatch.setattr(
        "orion.api.interactive.hompage_manager.homepage_manager.insight_generator.on_insight_consolidated_country",
    ...
    grouped = _run(homepage_manager.get_country_specific_insights())
    ...
    paginated = _run(homepage_manager.get_country_specific_insights_paginated("leak", "us", page=1, limit=1))
...
def test_homepage_country_and_display_helpers_cover_edge_cases():
    assert homepage_manager._country_category_to_index("social") == "social_model"
    assert homepage_manager._country_category_to_index("missing") is None
    assert homepage_manager._country_only_payload({"m_country": "US", "m_hash": "abc"}) == {"m_country": ["US"], "m_hash": "abc"}
    assert homepage_manager._country_matches(["PK, US"], "us") is True
    assert homepage_manager._country_matches("DE", "us") is False
    assert homepage_manager.parse_date_fallback("2026-04-01") == "April 01, 2026"

    display = homepage_manager.transform_for_display(
        "defacement_model",
        {
            "m_hash": "abc",
